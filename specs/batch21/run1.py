...
- Depth-recurrent transformer architecture (RECURRENT_BLOCKS, RECURRENT_LOOPS)
- Per-loop LoRA adapters on Q and V projections
- Per-loop signal vectors added to residual stream
- INT5+zstd22+late QAT compression
- Progressive auxiliary loss at intermediate recurrent loop iterations
- Loop curriculum: gradually increase active loops during early training
"""
...
class Hyperparameters:
    ...
    recurrent_blocks = int(os.environ.get("RECURRENT_BLOCKS", 0))
    recurrent_loops = int(os.environ.get("RECURRENT_LOOPS", 1))
    recurrent_lora_rank = int(os.environ.get("RECURRENT_LORA_RANK", 32))
    recurrent_curriculum = bool(int(os.environ.get("RECURRENT_CURRICULUM", "0")))
...
class LoopLoRA(nn.Module):
    """Per-loop low-rank adapter for a linear layer."""
    def __init__(self, in_features: int, out_features: int, rank: int):
        super().__init__()
        self.A = nn.Parameter(torch.zeros(rank, in_features))
        self.B = nn.Parameter(torch.zeros(out_features, rank))
        nn.init.kaiming_uniform_(self.A, a=math.sqrt(5))
        # B starts at zero so LoRA initially does nothing
...
class GPT(nn.Module):
    def __init__(
        ...
        if recurrent_blocks > 0:
            stored_layers = 1 + recurrent_blocks + 1  # prelude + core + coda
            self.effective_depth = 1 + recurrent_blocks * recurrent_loops + 1
        else:
        ...
        if recurrent_blocks > 0:
            # No U-net skip connections for recurrent architecture
        ...
        self.blocks = nn.ModuleList(
            [
                ...
                for i in range(stored_layers)
            ]
        )

        # Per-loop norms for recurrent blocks (cheap differentiation per loop iteration)
        if recurrent_blocks > 0 and recurrent_loops > 1:
            self.loop_attn_norms = nn.ModuleList([
                nn.ModuleList([RMSNorm() for _ in range(recurrent_blocks)])
                for _ in range(recurrent_loops)
            ])
            self.loop_mlp_norms = nn.ModuleList([
                nn.ModuleList([RMSNorm() for _ in range(recurrent_blocks)])
                for _ in range(recurrent_loops)
            ])
        else:
            self.loop_attn_norms = None
            self.loop_mlp_norms = None

        # Per-loop LoRA adapters for Q and V in each recurrent block
        kv_dim = num_kv_heads * (model_dim // num_heads)
        if recurrent_blocks > 0 and recurrent_loops > 1:
            lora_rank = recurrent_lora_rank
            self.loop_q_loras = nn.ModuleList([
                nn.ModuleList([LoopLoRA(model_dim, model_dim, lora_rank) for _ in range(recurrent_blocks)])
                for _ in range(recurrent_loops)
            ])
            self.loop_v_loras = nn.ModuleList([
                nn.ModuleList([LoopLoRA(model_dim, kv_dim, lora_rank) for _ in range(recurrent_blocks)])
                for _ in range(recurrent_loops)
            ])
            # Per-loop signal vectors added to residual stream
            self.loop_signals = nn.Parameter(torch.zeros(recurrent_loops, model_dim))
        else:
            self.loop_q_loras = None
            self.loop_v_loras = None
            self.loop_signals = None
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if self.recurrent_blocks > 0:
            # --- Depth-recurrent forward ---
            # Prelude (block 0)
            qd = lora.q_loras[0] if lora else None
            vd = lora.v_loras[0] if lora else None
            x = self.blocks[0](x, x0, qd, vd)

            # Use active_loops (set by curriculum) or fall back to recurrent_loops
            active_loops = getattr(self, 'active_loops', self.recurrent_loops)

            # Recurrent core: blocks 1..recurrent_blocks looped active_loops times
            for loop in range(active_loops):
                # Add loop signal to residual stream
                if self.loop_signals is not None:
                    x = x + self.loop_signals[loop].to(dtype=x.dtype)[None, None, :]

                for b in range(self.recurrent_blocks):
                    block = self.blocks[1 + b]

                    # Create LoRA delta functions for this loop+block
                    if self.loop_q_loras is not None:
                        q_lora = self.loop_q_loras[loop][b]
                        v_lora = self.loop_v_loras[loop][b]
                        q_delta_fn = lambda n, ql=q_lora: ql(n)
                        v_delta_fn = lambda n, vl=v_lora: vl(n)
                    else:
                        q_delta_fn = None
                        v_delta_fn = None

                    # Swap in per-loop norms if we have multiple loops
                    if self.loop_attn_norms is not None:
                        orig_attn_norm = block.attn_norm
                        orig_mlp_norm = block.mlp_norm
                        block.attn_norm = self.loop_attn_norms[loop][b]
                        block.mlp_norm = self.loop_mlp_norms[loop][b]

                    # LoRA from TTT: use block index within stored blocks
                    if lora is not None:
                        ttt_qd = lora.q_loras[1 + b]
                        ttt_vd = lora.v_loras[1 + b]
                        # Combine TTT LoRA with loop LoRA
                        if q_delta_fn is not None:
                            orig_q_delta_fn = q_delta_fn
                            orig_v_delta_fn = v_delta_fn
                            q_delta_fn = lambda n, tqd=ttt_qd, oqd=orig_q_delta_fn: oqd(n) + tqd(n)
                            v_delta_fn = lambda n, tvd=ttt_vd, ovd=orig_v_delta_fn: ovd(n) + tvd(n)
                        else:
                            q_delta_fn = ttt_qd
                            v_delta_fn = ttt_vd

                    x = block(x, x0, q_delta_fn, v_delta_fn)

                    if self.loop_attn_norms is not None:
                        block.attn_norm = orig_attn_norm
                        block.mlp_norm = orig_mlp_norm

                # Progressive auxiliary loss: compute intermediate loss after each loop (except the last)
                if self.training and loop < active_loops - 1:
                    x_aux = self.final_norm(x)
                    if self.tie_embeddings:
                        aux_logits = F.linear(x_aux, self.tok_emb.weight)
                    else:
                        aux_logits = self.lm_head(x_aux)
                    aux_logits = self.logit_softcap * torch.tanh(aux_logits / self.logit_softcap)
                    loop_loss = F.cross_entropy(
                        aux_logits.float().reshape(-1, aux_logits.size(-1)),
                        target_ids.reshape(-1),
                        reduction="mean",
                    )
                    # Weight increases for later loops: 0.1 * (loop+1) / active_loops
                    weight = 0.1 * (loop + 1) / active_loops
                    aux_loss = aux_loss + weight * loop_loss

            # Coda (last block)
            qd = lora.q_loras[-1] if lora else None
            vd = lora.v_loras[-1] if lora else None
            x = self.blocks[-1](x, x0, qd, vd)
        else:
        ...
        if self.training and self.recurrent_blocks > 0 and isinstance(aux_loss, torch.Tensor):
...
def main() -> None:
    ...
    # Compute stored layers for recurrent mode
    if args.recurrent_blocks > 0:
        stored_layers = 1 + args.recurrent_blocks + 1
    else:
        stored_layers = args.num_layers

    base_model = GPT(
        vocab_size=args.vocab_size,
        num_layers=stored_layers,
        ...
        recurrent_lora_rank=args.recurrent_lora_rank,
    ).to(device).bfloat16()
    ...
    if args.recurrent_blocks > 0:
        log0(f"recurrence:blocks={args.recurrent_blocks} loops={args.recurrent_loops} lora_rank={args.recurrent_lora_rank} aux_loss:enabled curriculum:{args.recurrent_curriculum}")
    else:
        log0(f"recurrence:blocks={args.recurrent_blocks} loops={args.recurrent_loops} effective_depth={base_model.effective_depth} stored_layers={len(list(base_model.blocks))}")
    ...
    # Include per-loop norm parameters in scalar_params (they have no learnable weight,
    # but RMSNorm is parameterless so this is a no-op; included for future-proofing)
    if base_model.loop_attn_norms is not None:
        for loop_norms in base_model.loop_attn_norms:
            for norm in loop_norms:
                for p in norm.parameters():
                    scalar_params.append(p)
    if base_model.loop_mlp_norms is not None:
        for loop_norms in base_model.loop_mlp_norms:
            for norm in loop_norms:
                for p in norm.parameters():
                    scalar_params.append(p)

    # Include per-loop LoRA parameters in scalar_params (trained with Adam, NOT Muon)
    if base_model.loop_q_loras is not None:
        for lora_list in base_model.loop_q_loras:
            for lora in lora_list:
                scalar_params.extend(list(lora.parameters()))
    if base_model.loop_v_loras is not None:
        for lora_list in base_model.loop_v_loras:
            for lora in lora_list:
                scalar_params.extend(list(lora.parameters()))
    # Include per-loop signal vectors in scalar_params
    if base_model.loop_signals is not None:
        scalar_params.append(base_model.loop_signals)
    ...
    while True:
        ...
        # Loop curriculum: gradually increase active loops over first 25% of training
        if args.recurrent_blocks > 0 and args.recurrent_curriculum:
            frac = min(elapsed_ms / max_wallclock_ms, 1.0) if max_wallclock_ms else min(step / args.iterations, 1.0)
            target_loops = args.recurrent_loops
            active_loops = max(1, int(1 + (target_loops - 1) * min(frac / 0.25, 1.0)))
        ...
        if should_log_train:
            ...
            if args.recurrent_blocks > 0 and args.recurrent_curriculum:
                log_msg += f" active_loops:{getattr(base_model, 'active_loops', args.recurrent_loops)}"
            log0(log_msg)

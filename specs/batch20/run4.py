...
- INT5 late QAT (fake quantization after 50% training)
- Mixture-of-Experts MLP option (top-k routing)
"""
...
class Hyperparameters:
    ...
    # MoE MLP settings
    use_moe_mlp = bool(int(os.environ.get("USE_MOE_MLP", "0")))
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    # Handle 3D tensors (MoE expert weights: E, out, in)
                    # by reshaping to 2D for Newton-Schulz, then back
                    if g.ndim == 3:
                        orig_shape = g.shape
                        g_2d = g.reshape(g.shape[0] * g.shape[1], g.shape[2])
                        g_2d = zeropower_via_newtonschulz5(g_2d, steps=backend_steps)
                        g_2d *= max(1, g_2d.size(0) / g_2d.size(1)) ** 0.5
                        updates_flat[curr : curr + p.numel()] = g_2d.reshape(-1)
                    else:
...
def _quantize_3d_tensor_int5(t: Tensor) -> tuple[Tensor, Tensor]:
    """Quantize a 3D tensor (E, out, in) by treating each (E, out) slice as a row."""
    t32 = t.float()
    orig_shape = t32.shape
    t2d = t32.reshape(-1, orig_shape[-1])  # (E*out, in)
    clip_abs = (
        torch.quantile(t2d.abs(), INT8_CLIP_Q, dim=1)
        if t2d.numel()
        else torch.empty((t2d.shape[0],), dtype=torch.float32)
    )
    clipped = torch.maximum(torch.minimum(t2d, clip_abs[:, None]), -clip_abs[:, None])
    scale = (clip_abs / INT5_QUANT_RANGE).clamp_min(1.0 / INT5_QUANT_RANGE)
    q = torch.clamp(torch.round(clipped / scale[:, None]), -16, INT5_QUANT_RANGE).to(torch.int8).contiguous()
    # Store as 2D for compatibility with dequantization
    return q, scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()


def _quantize_3d_tensor_int8(t: Tensor) -> tuple[Tensor, Tensor]:
    """Quantize a 3D tensor (E, out, in) by treating each (E, out) slice as a row."""
    t32 = t.float()
    orig_shape = t32.shape
    t2d = t32.reshape(-1, orig_shape[-1])  # (E*out, in)
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if t.ndim == 3:
            # 3D MoE expert weights: quantize as reshaped 2D
            q, s = _quantize_3d_tensor_int8(t)
            qmeta[name] = {"scheme": "per_row", "axis": 0, "orig_shape": list(t.shape)}
        else:
    ...
    for name, tensor in state_dict.items():
        ...
        if t.ndim == 3:
            ...
            if use_int8:
                ...
                q, s = _quantize_3d_tensor_int5(t)
            qmeta[name] = {"scheme": "per_row", "axis": 0, "bits": 8 if use_int8 else 5, "orig_shape": list(t.shape)}
        else:
            if use_int8:
                q, s = quantize_float_tensor(t)
            else:
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        # Restore original 3D shape for MoE expert weights
        orig_shape = meta.get("orig_shape")
        if orig_shape is not None:
            deq = deq.reshape(orig_shape).contiguous()
        out[name] = deq
...
class MoEMLP(nn.Module):
    ...
    Compile-friendly implementation: computes all expert outputs and selects
    top-k via gather. Suitable for small num_experts (4-8).
    """
    def __init__(self, dim: int, hidden_mult: int, num_experts: int, top_k: int = 2):
        super().__init__()
        self.num_experts = num_experts
        self.top_k = top_k
        hidden = dim * hidden_mult

        # Router: simple linear projection to expert logits
        self.router = nn.Linear(dim, num_experts, bias=False)

        # Expert weights: each expert is a small MLP (up + down projection)
        # expert_up: (E, hidden, dim) — projects input to hidden
        # expert_down: (E, dim, hidden) — projects hidden back to dim
        self.expert_up = nn.Parameter(torch.empty(num_experts, hidden, dim))
        self.expert_down = nn.Parameter(torch.empty(num_experts, dim, hidden))

        # Initialize each expert
        for i in range(num_experts):
            nn.init.kaiming_uniform_(self.expert_up[i], a=math.sqrt(5))
            nn.init.zeros_(self.expert_down[i])

        self.aux_loss_coeff = 0.01  # load balancing coefficient
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Route: compute expert selection probabilities
        router_logits = self.router(x_flat.float())  # (N, E)
        router_probs = F.softmax(router_logits, dim=-1)
        topk_weights, topk_ids = torch.topk(router_probs, self.top_k, dim=-1)  # (N, k)
        topk_weights = (topk_weights / topk_weights.sum(dim=-1, keepdim=True)).to(dtype)

        # Compute ALL expert outputs (simple, compile-friendly)
        # expert_up: (E, hidden, dim), expert_down: (E, dim, hidden)
        up_w = self.expert_up.to(dtype)
        down_w = self.expert_down.to(dtype)

        # Apply QAT fake quantization to expert weights if active
        if self._qat_active and self.training:
            up_w = self._fake_quant_3d(up_w)
            down_w = self._fake_quant_3d(down_w)

        all_h = torch.einsum('nd,ehd->neh', x_flat, up_w)  # (N, E, hidden)
        all_h = F.relu(all_h).square()
        all_out = torch.einsum('neh,edh->ned', all_h, down_w)  # (N, E, dim)

        # Gather top-k expert outputs and weight them
        # topk_ids: (N, k), all_out: (N, E, dim)
        selected = torch.gather(
            all_out, 1,
            topk_ids.unsqueeze(-1).expand(-1, -1, orig_shape[-1])
        )  # (N, k, dim)
        output = (selected * topk_weights.unsqueeze(-1)).sum(dim=1)  # (N, dim)

        # Load balancing auxiliary loss
        if self.training:
            self._aux_loss = self.aux_loss_coeff * self.num_experts * (router_probs.mean(0) ** 2).sum()

        return output.reshape(orig_shape)

    def _fake_quant_3d(self, w: Tensor) -> Tensor:
        """STE fake quantization for 3D expert weight tensors."""
        with torch.no_grad():
            w32 = w.float()
            # Reshape to 2D for per-row quantile computation
            orig_shape = w32.shape
            w2d = w32.reshape(-1, orig_shape[-1])
            clip_abs = torch.quantile(w2d.abs(), INT8_CLIP_Q, dim=1).clamp_min(1e-8)
            scale = clip_abs / INT5_QUANT_RANGE
            w_clipped = torch.clamp(w2d, -clip_abs[:, None], clip_abs[:, None])
            w_q = (torch.round(w_clipped / scale[:, None]) * scale[:, None])
            w_q = w_q.reshape(orig_shape).to(w.dtype)
...
class Block(nn.Module):
    def __init__(
        ...
        if use_moe_mlp:
            self.moe_mlp = MoEMLP(dim, mlp_mult, moe_num_experts, moe_top_k)
        else:
    ...
    def forward(self, x: Tensor, x0: Tensor, q_delta_fn=None, v_delta_fn=None) -> Tensor:
        ...
        mlp_module = self.moe_mlp if self.use_moe_mlp else self.mlp
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * mlp_module(self.mlp_norm(x))
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if lora:
            ...
            loss = F.cross_entropy(logits.float().reshape(-1, logits.size(-1)), target_ids.reshape(-1), reduction="mean")

        # Add MoE auxiliary losses for load balancing
        if self.use_moe_mlp and self.training:
            for block in self.blocks:
                if hasattr(block, 'moe_mlp') and block.moe_mlp._aux_loss is not None:
                    loss = loss + block.moe_mlp._aux_loss
...
def main() -> None:
    ...
    # Use find_unused_parameters=True when MoE is enabled (router may not use all experts)
    ...
    # For MoE: expert_up and expert_down are 3D tensors that go to Muon (matrix_params)
    # Router weights are 2D and also go to matrix_params (or scalar if 1D)
    matrix_params = [
        p
    ...
    if args.use_moe_mlp:
        log0(f"moe_mlp:enabled num_experts:{args.moe_num_experts} top_k:{args.moe_top_k}")
    else:
        log0("moe_mlp:disabled")
    ...
    while True:
        ...
        if frac_done >= 0.5:
            ...
            # Also activate QAT on MoE expert weights
            if args.use_moe_mlp:
                for m in base_model.modules():
                    if isinstance(m, MoEMLP) and not m._qat_active:
                        m._qat_active = True
                        log0(f"late_qat_moe_switchover_step:{step}")
        zero_grad_all()
    ...
    # Roundtrip validation using INT5
    if distributed:
        dist.barrier()

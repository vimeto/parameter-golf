...
# ---------------------------------------------------------------------------
# Score-First TTT (PR #549 protocol)
# ---------------------------------------------------------------------------
...
def eval_val_ttt_scorefirst(
    ...
    """Score-first TTT: for each chunk, score it first, then train on it.
    Chunk N is scored by model adapted on chunks 0..N-1."""
    CHUNK_TOKENS = 32768
    TTT_EPOCHS = 5
    TTT_LR = 0.002
    TTT_GRAD_CLIP = 1.0
    ...
    if log_fn:
        log_fn(f"ttt:score_first chunks={num_chunks} my_range={my_chunk_start}..{my_chunk_end} "
               f"lr=0.002 epochs=5 freeze=0 optimizer=sgd")

    # Save original state for potential reset
    original_state = {k: v.detach().cpu().clone() for k, v in eval_model.state_dict().items()}

    # Freeze blocks if requested
    if FREEZE_BLOCKS > 0:
        for i in range(min(FREEZE_BLOCKS, len(eval_model.blocks))):
            for p in eval_model.blocks[i].parameters():
                p.requires_grad_(False)

    # Enable gradients for TTT fine-tuning on non-frozen params
    for name, p in eval_model.named_parameters():
        if p.requires_grad is not False:  # already frozen blocks stay frozen
            p.requires_grad_(True)

    ttt_optimizer = torch.optim.SGD(
        [p for p in eval_model.parameters() if p.requires_grad],
    ...
    for ci in range(my_chunk_start, my_chunk_end):
        ...
        eval_model.eval()
        with torch.inference_mode():
            # Process chunk in sub-sequences of seq_len
        ...
        # --- STEP 2: TRAIN on this chunk (adapt for future chunks) ---
        eval_model.train()
        for epoch in range(TTT_EPOCHS):
            for pos in range(0, chunk_len, seq_len):
                ...
                train_loss.backward()
                if TTT_GRAD_CLIP > 0:
                    torch.nn.utils.clip_grad_norm_([p for p in eval_model.parameters() if p.requires_grad], TTT_GRAD_CLIP)
                ttt_optimizer.step()
    ...
    # Restore original weights (so sliding eval can run uncontaminated if needed)
    eval_model.load_state_dict(original_state, strict=True)
    eval_model.eval()

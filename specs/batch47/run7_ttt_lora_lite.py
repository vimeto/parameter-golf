...
FULL_GPTQ = False  # Disabled for GPTQ-lite only TTT test
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if HAS_FLASH3 and not IS_ROCM:
            ...
            # SDPA expects (B, H, T, D), flash_attn uses (B, T, H, D)
...
# ---------------------------------------------------------------------------
# Score-First TTT with LoRA via forward hooks (PR #549 protocol)
# ---------------------------------------------------------------------------

class TTTLoRALinear(nn.Module):
    """Low-rank adapter for TTT fine-tuning."""
    def __init__(self, in_features: int, out_features: int, lora_rank: int = 8):
        super().__init__()
        self.A = nn.Parameter(torch.randn(lora_rank, in_features) * (1.0 / math.sqrt(in_features)))
...
class TTTLoRAAdapter(nn.Module):
    """LoRA adapters on Q and V projections for all blocks, injected via hooks."""
    def __init__(self, model: GPT, lora_rank: int = 8):
        super().__init__()
        ...
        for block in model.blocks:
            ...
            self.q_loras.append(TTTLoRALinear(dim, q_out, lora_rank))
            self.v_loras.append(TTTLoRALinear(dim, v_out, lora_rank))
        self._hooks: list = []

    def install_hooks(self, model: GPT) -> None:
        """Register forward hooks on c_q and c_v to add LoRA deltas."""
        self.remove_hooks()
        for i, block in enumerate(model.blocks):
            q_lora = self.q_loras[i]
            v_lora = self.v_loras[i]
            def make_hook(lora_mod):
                def hook_fn(module, input, output):
                    return output + lora_mod(input[0])
                return hook_fn
            self._hooks.append(block.attn.c_q.register_forward_hook(make_hook(q_lora)))
            self._hooks.append(block.attn.c_v.register_forward_hook(make_hook(v_lora)))
...
def eval_val_ttt_scorefirst(
    ...
    """Score-first TTT with LoRA: for each chunk, score it first, then train LoRA on it.
    Chunk N is scored by model + LoRA adapted on chunks 0..N-1."""
    CHUNK_TOKENS = 32768
    TTT_EPOCHS = 3
    TTT_LR = 0.003
    TTT_GRAD_CLIP = 1.0
    ...
    total_tokens = val_tokens.numel() - 1  # -1 for next-token prediction
    num_chunks = (total_tokens + CHUNK_TOKENS - 1) // CHUNK_TOKENS

    # Distribute chunks across ranks
    my_chunk_start = (num_chunks * rank) // world_size
    my_chunk_end = (num_chunks * (rank + 1)) // world_size

    if log_fn:
        log_fn(f"ttt:score_first_lora chunks={num_chunks} my_range={my_chunk_start}..{my_chunk_end} "
               f"lr=0.003 epochs=3 freeze=0 optimizer=sgd lora_rank={LORA_RANK}")

    # Freeze all model params -- only train LoRA
    for p in eval_model.parameters():
        p.requires_grad_(False)

    # Create LoRA adapter and install hooks
    lora_adapter = TTTLoRAAdapter(eval_model, lora_rank=LORA_RANK).to(device)
    lora_adapter.install_hooks(eval_model)
    ...
    for ci in range(my_chunk_start, my_chunk_end):
        ...
        # --- STEP 1: SCORE this chunk in inference mode ---
        # Hooks are active so LoRA deltas are applied during scoring
        eval_model.eval()
        lora_adapter.eval()
        with torch.inference_mode():
            for pos in range(0, chunk_len, seq_len):
                end_pos = min(pos + seq_len, chunk_len)
                sub_len = end_pos - pos
                if sub_len < 1:
                    continue
                token_offset = chunk_start + pos
                chunk_tokens_slice = val_tokens[token_offset:token_offset + sub_len + 1].to(
                    device=device, dtype=torch.int64)
                x = chunk_tokens_slice[:-1].unsqueeze(0)  # [1, sub_len]
                y = chunk_tokens_slice[1:].unsqueeze(0)   # [1, sub_len]
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
        ...
        # --- STEP 2: TRAIN LoRA on this chunk (adapt for future chunks) ---
        eval_model.train()
        lora_adapter.train()
        for epoch in range(TTT_EPOCHS):
            for pos in range(0, chunk_len, seq_len):
                ...
                chunk_tokens_slice = val_tokens[token_offset:token_offset + sub_len + 1].to(
                    device=device, dtype=torch.int64)
                x = chunk_tokens_slice[:-1].unsqueeze(0)
                y = chunk_tokens_slice[1:].unsqueeze(0)
                ttt_optimizer.zero_grad()
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                    train_loss = eval_model(x, y)
                train_loss.backward()
                if TTT_GRAD_CLIP > 0:
                    torch.nn.utils.clip_grad_norm_(lora_adapter.parameters(), TTT_GRAD_CLIP)
                ttt_optimizer.step()

        if log_fn and (ci - my_chunk_start) % 10 == 0:
            avg_loss = loss_sum.item() / max(token_count.item(), 1.0)
            log_fn(f'ttt:chunk {ci}/{my_chunk_end} avg_loss_so_far:{avg_loss:.4f}')

    # Clean up: remove hooks, restore model state
    lora_adapter.remove_hooks()
    ...
    val_loss = float(loss_sum.item() / max(token_count.item(), 1.0))
    bpb = float((loss_sum.item() / math.log(2.0)) / max(byte_count.item(), 1.0))

    # Re-enable all gradients to default state
    for p in eval_model.parameters():
...
def main() -> None:
    ...
    ttt_val_loss, ttt_val_bpb = eval_val_ttt_scorefirst(
    ...
    torch.cuda.synchronize()
    log0(
        f'ttt_scorefirst val_loss:{ttt_val_loss:.4f} ttt_bpb:{ttt_val_bpb:.4f} '
        f'eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms'
    )
    log0(f'ttt_scorefirst_exact val_loss:{ttt_val_loss:.8f} ttt_bpb:{ttt_val_bpb:.8f}')

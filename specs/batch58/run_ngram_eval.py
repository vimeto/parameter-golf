...
class Hyperparameters:
    ...
    ngram_buckets = int(os.environ.get("NGRAM_BUCKETS", 4194304))  # 2^22
    ngram_alpha_low = float(os.environ.get("NGRAM_ALPHA_LOW", 0.05))
    ngram_alpha_high = float(os.environ.get("NGRAM_ALPHA_HIGH", 0.55))
    ngram_entropy_thresh = float(os.environ.get("NGRAM_ENTROPY_THRESH", 4.0))
...
def eval_val_sliding_ngram(
    ...
    batch_seqs: int = 32,
    eval_seq_len: int | None = None,
    ngram_orders: tuple[int, ...] = (2, 3, 4, 5, 6, 7),
    ngram_buckets: int = 4_194_304,
    alpha_low: float = 0.05,
    alpha_high: float = 0.55,
    entropy_thresh: float = 4.0,
    log_fn=print,
) -> tuple[float, float]:
    """Sliding window eval with backward-looking n-gram backoff mixing.

    Core idea: maintain n-gram count tables from already-scored tokens.
    At each position, mix the neural model probability with n-gram probability
    using entropy-adaptive alpha. Higher entropy -> rely more on n-gram cache
    (when model is uncertain, n-gram patterns help more).
    """
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    val_np = val_tokens.numpy()

    PRIMES = np.array([1, 2654435761, 805459861, 3674653429, 2246822519, 1543895933, 3513636451], dtype=np.uint64)
    max_order = max(ngram_orders)

    # Per-order count arrays on CPU
    ctx_counts = [np.zeros(ngram_buckets, dtype=np.int32) for _ in range(max_order + 1)]
    tok_counts = [np.zeros(ngram_buckets, dtype=np.int32) for _ in range(max_order + 1)]

    def hash_context(pos: int, order: int) -> int | None:
        """Hash the (order-1) context tokens before position pos."""
        h = np.uint64(0)
        for k in range(1, order):
            if pos - k < 0:
                return None
            h ^= np.uint64(int(val_np[pos - k])) * PRIMES[k % len(PRIMES)]
        return int(h % np.uint64(ngram_buckets))

    def hash_context_token(ctx_hash: int, token: int) -> int:
        """Hash context + the specific token."""
        h2 = (np.uint64(ctx_hash) ^ (np.uint64(int(token)) * PRIMES[0])) % np.uint64(ngram_buckets)
        return int(h2)

    def add_to_cache(pos: int) -> None:
        """Add the token at pos to all n-gram orders."""
        token = int(val_np[pos])
        for order in ngram_orders:
            ch = hash_context(pos, order)
            if ch is not None:
                ctx_counts[order][ch] += 1
                cth = hash_context_token(ch, token)
                tok_counts[order][cth] += 1

    def query_ngram(pos: int) -> tuple[float | None, int]:
        """Query the n-gram cache with backoff. Returns (prob, order_used) or (None, 0)."""
        token = int(val_np[pos])
        for order in sorted(ngram_orders, reverse=True):  # highest order first
            ch = hash_context(pos, order)
            if ch is None or ctx_counts[order][ch] == 0:
                continue
            cth = hash_context_token(ch, token)
            count = tok_counts[order][cth]
            total = ctx_counts[order][ch]
            if total > 0:
                return count / total, order
        return None, 0

    # Build window list (same as eval_val_sliding)
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= 1]
    total_windows = len(window_starts)
    my_s = (total_windows * rank) // world_size
    ...
    # Also track pure model BPB for comparison
    model_loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    ...
    # Track which absolute positions have been scored (to avoid double-counting)
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            # Get full log-probs for n-gram mixing
            ...
            for i, ws in enumerate(batch_ws):
                ...
                for t in range(s, wlen):
                    abs_pos = ws + t + 1  # absolute position of predicted token
                    if abs_pos in scored_positions:
                        continue
                    scored_positions.add(abs_pos)

                    token = int(val_np[abs_pos])
                    prev_token = int(val_np[abs_pos - 1])

                    # Model NLL
                    model_nll = nll[i, t].item()
                    model_loss_sum += model_nll

                    # N-gram query
                    ngram_prob, ngram_order = query_ngram(abs_pos)
                    ngram_total += 1

                    if ngram_prob is not None and ngram_prob > 0:
                        ngram_hits += 1
                        # Entropy-adaptive mixing
                        lp = log_probs[i, t]  # (vocab,)
                        entropy = -(lp * lp.exp()).sum().item()
                        # Sigmoid-based alpha: higher entropy -> higher alpha
                        alpha = alpha_low + alpha_high * (1.0 / (1.0 + math.exp(-2.0 * (entropy - entropy_thresh))))

                        # Mix: log((1-alpha)*p_model + alpha*p_ngram)
                        model_prob = math.exp(-model_nll)
                        mixed_prob = (1.0 - alpha) * model_prob + alpha * ngram_prob
                        final_nll = -math.log(max(mixed_prob, 1e-30))
                    else:
                        final_nll = model_nll

                    # BPB calculation
                    tok_bytes = float(base_bytes_lut[token].item())
                    if has_leading_space_lut[token].item() and not is_boundary_token_lut[prev_token].item():
                        tok_bytes += 1.0

                    loss_sum += final_nll
                    token_count += 1
                    byte_count += tok_bytes

                    # Add this token to the n-gram cache (backward-looking: only after scoring)
                    add_to_cache(abs_pos)

            # Progress logging
            if rank == 0 and bi > 0 and bi % (batch_seqs * 50) == 0:
                elapsed = time.perf_counter() - t0
                cur_bpb = (loss_sum.item() / math.log(2.0)) / max(byte_count.item(), 1) if token_count.item() > 0 else 0.0
                hit_rate = ngram_hits / max(ngram_total, 1)
                log_fn(f"  ngram_sliding [{bi}/{len(my_windows)}] bpb={cur_bpb:.6f} "
                       f"ngram_hit_rate={hit_rate:.4f} time={elapsed:.1f}s")

    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(byte_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(model_loss_sum, op=dist.ReduceOp.SUM)

    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    model_val_loss = (model_loss_sum / token_count).item()
    model_bpb = model_val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    hit_rate = ngram_hits / max(ngram_total, 1)

    base_model.train()
    elapsed = time.perf_counter() - t0
    log_fn(f"ngram_sliding:done mixed_bpb={val_bpb:.6f} model_only_bpb={model_bpb:.6f} "
           f"ngram_hit_rate={hit_rate:.4f} tokens={int(token_count.item())} "
...
def main() -> None:
    ...
    if args.skip_training:
        # === EVAL-ONLY MODE: Load pre-quantized model from MODEL_PATH ===
        if not args.model_path:
            raise ValueError("SKIP_TRAINING=1 requires MODEL_PATH to be set")
        log0(f"skip_training:loading model from {args.model_path}")

        # Load code for size calculation
        if args.code_path and os.path.exists(args.code_path):
            code = Path(args.code_path).read_text(encoding="utf-8")
            log0(f"skip_training:loaded code from {args.code_path} ({len(code.encode('utf-8'))} bytes)")

        # Read and decompress the quantized model
        with open(args.model_path, "rb") as f:
            quant_blob_disk = f.read()
        model_file_bytes = len(quant_blob_disk)
        code_bytes = len(code.encode("utf-8"))
        log0(f"skip_training:model_file_bytes:{model_file_bytes} code_bytes:{code_bytes} "
             f"total_artifact:{model_file_bytes + code_bytes}")

        quant_state = torch.load(
            io.BytesIO(lzma.decompress(quant_blob_disk) if _COMPRESSOR == "lzma"
                       else zstandard.ZstdDecompressor().decompress(quant_blob_disk) if _COMPRESSOR == "zstd"
                       else zlib.decompress(quant_blob_disk)),
            map_location="cpu",
        )

        # Create a fresh model for shape reference (needed for rebanking)
        ref_model = GPT(
        ...
        unbanked_ref = _unbank_state_dict(ref_sd, args.num_layers)
        del ref_model

        deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], unbanked_ref)
        deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, ref_sd)
        ...
        if args.ngram_eval_enabled:
            ngram_orders_parsed = tuple(int(x) for x in args.ngram_orders.split(","))
            ngram_stride = args.eval_stride if args.eval_stride > 0 else 64
            torch.cuda.synchronize()
            t_ngram = time.perf_counter()
            ng_val_loss, ng_val_bpb = eval_val_sliding_ngram(
                args, eval_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                stride=ngram_stride,
                eval_seq_len=sw_seq_len,
                ngram_orders=ngram_orders_parsed,
                ngram_buckets=args.ngram_buckets,
                alpha_low=args.ngram_alpha_low,
                alpha_high=args.ngram_alpha_high,
                entropy_thresh=args.ngram_entropy_thresh,
                log_fn=log0,
            )
            torch.cuda.synchronize()
            log0(
                f"final_ngram_sliding val_loss:{ng_val_loss:.4f} val_bpb:{ng_val_bpb:.4f} "
                f"stride:{ngram_stride} eval_time:{1000.0 * (time.perf_counter() - t_ngram):.0f}ms"
            )
            log0(f"final_ngram_sliding_exact val_loss:{ng_val_loss:.8f} val_bpb:{ng_val_bpb:.8f}")

...
def main() -> None:
    ...
    if master_process:
        ...
        # Save to persistent scratch for eval-only jobs
        import shutil
        scratch_model_dir = os.environ.get("SCRATCH_MODEL_DIR", "/scratch/project_462001163/vtoivone/pgolf_cache/models")
        os.makedirs(scratch_model_dir, exist_ok=True)
        scratch_path = os.path.join(scratch_model_dir, f"model_{args.run_id}.int6.ptz")
        shutil.copy2("final_model.int6.ptz", scratch_path)
        log0(f"model_saved_to_scratch:{scratch_path} size:{os.path.getsize(scratch_path)}")
        # Save code alongside
        code_path = os.path.join(scratch_model_dir, f"code_{args.run_id}.py")
        with open(code_path, "w") as f:
            f.write(code)
        log0(f"code_saved_to_scratch:{code_path}")
    if distributed:
        dist.barrier()

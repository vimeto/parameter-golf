    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                # Soft rounding: differentiable approximation of round()
                w_scaled = w32 / scale[:, None]
                w_floor = torch.floor(w_scaled)
                r = w_scaled - w_floor - 0.5  # centered fractional part
                alpha = CastedLinear._soft_round_alpha
                tanh_half = torch.tanh(torch.tensor(alpha / 2.0))
                w_soft = w_floor + 0.5 * torch.tanh(alpha * r) / tanh_half + 0.5
                w_q = (torch.clamp(w_soft, -32, 31) * scale[:, None]).to(x.dtype)
            w = w + (w_q - w).detach()
...
def main() -> None:
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            CastedLinear._qat_start_scale = scale
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        # Anneal soft_round_alpha from 1.0 to 16.0 during QAT phase
        if CastedLinear._qat_enabled and hasattr(CastedLinear, '_qat_start_scale'):
            qat_progress = 1.0 - (scale / CastedLinear._qat_start_scale) if CastedLinear._qat_start_scale > 0 else 1.0

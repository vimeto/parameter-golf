...
class Block(nn.Module):
    def __init__(
        ...
        self.resid_mix = nn.Parameter(torch.stack((
            torch.ones(dim), torch.zeros(dim), torch.zeros(dim), torch.zeros(dim)
        )).float())  # 4-source: [current_x, prev[-1], prev[-2], x0]
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, prev_outputs: list[Tensor] | None = None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[3][None, None, :] * x0
        if prev_outputs is not None and len(prev_outputs) >= 1:
            x_in = x_in + mix[1][None, None, :] * prev_outputs[-1]
        if prev_outputs is not None and len(prev_outputs) >= 2:
            x_in = x_in + mix[2][None, None, :] * prev_outputs[-2]
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        skips: list[Tensor] = []
        prev_outputs: list[Tensor] = []
        ve_cache: dict = {}
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x = self.blocks[i](x, x0, v_embed=ve, prev_outputs=prev_outputs)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, v_embed=ve, prev_outputs=prev_outputs)
            prev_outputs.append(x)
        x = self.final_norm(x)

...
- BitNet b1.58 ternary QAT: {-1, 0, +1} quantized weights via AbsMedian + STE
"""
...
# Feature flags
USE_TERNARY_QAT = bool(int(os.environ.get("USE_TERNARY_QAT", "0")))
...
def pack_ternary(tensor: Tensor) -> tuple[bytes, Tensor, int]:
    """Pack ternary {-scale, 0, +scale} into 2 bits per value.
    Returns packed bytes, per-row scales, and original numel."""
    t = tensor.float()
    # Compute per-row scale
    if t.ndim == 2:
        scale = t.abs().mean(dim=1).clamp_min(1e-8)
        # Quantize to ternary
        t_norm = t / scale[:, None]
        ternary = torch.zeros_like(t_norm, dtype=torch.int8)
        ternary[t_norm > 0.5] = 1
        ternary[t_norm < -0.5] = -1
    else:
        scale = t.abs().mean().clamp_min(1e-8).unsqueeze(0)
        t_norm = t / scale
        ternary = torch.zeros_like(t_norm, dtype=torch.int8)
        ternary[t_norm > 0.5] = 1
        ternary[t_norm < -0.5] = -1

    # Pack: encode -1->2, 0->0, +1->1, pack 4 values per byte
    flat = ternary.reshape(-1).numpy().astype(np.int8)
    encoded = np.where(flat == -1, 2, np.where(flat == 1, 1, 0)).astype(np.uint8)
    ...
    # Pack 4 values into 1 byte: v0 in bits 6-7, v1 in bits 4-5, v2 in bits 2-3, v3 in bits 0-1
    packed = (encoded[0::4] << 6) | (encoded[1::4] << 4) | (encoded[2::4] << 2) | encoded[3::4]

    return bytes(packed), scale.to(torch.float16), n


def unpack_ternary(data: bytes, scale: Tensor, orig_len: int, shape: tuple) -> Tensor:
    """Unpack 2-bit ternary values back to float tensor."""
    packed = np.frombuffer(data, dtype=np.uint8)
    v0 = (packed >> 6) & 0x03
    v1 = (packed >> 4) & 0x03
    v2 = (packed >> 2) & 0x03
    v3 = packed & 0x03
    encoded = np.stack([v0, v1, v2, v3], axis=1).reshape(-1)[:orig_len]

    # Decode: 0->0, 1->+1, 2->-1
    ternary = np.where(encoded == 2, -1, np.where(encoded == 1, 1, 0)).astype(np.float32)
    t = torch.tensor(ternary).reshape(shape)

    # Apply per-row scale
    s = scale.float()
    if s.ndim > 0 and s.numel() > 1 and t.ndim == 2:
        return (t * s.view(-1, 1)).contiguous()
    elif s.numel() == 1:
        return (t * s.item()).contiguous()
    else:
        return (t * s.view(-1, 1)).contiguous()


def quantize_state_dict_ternary(state_dict: dict[str, Tensor]):
    """Ternary quantization: 2 bits per weight for large matrices, INT8 for embeddings."""
    ternary_packed: dict[str, bytes] = {}
    ternary_scales: dict[str, Tensor] = {}
    ternary_shapes: dict[str, tuple] = {}
    ternary_lens: dict[str, int] = {}
    passthrough: dict[str, Tensor] = {}
    passthrough_orig_dtypes: dict[str, str] = {}
    stats = {"param_count": 0, "ternary_bytes": 0, "baseline_bytes": 0}
    ...
    for name, tensor in state_dict.items():
        ...
        if not t.is_floating_point() or t.numel() <= 65536:
            # Small tensors: keep as float16
            if t.is_floating_point():
                if t.dtype in (torch.float32, torch.bfloat16):
                    t = t.to(torch.float16)
            passthrough[name] = t
            stats["ternary_bytes"] += t.numel() * t.element_size()
            continue

        # Embeddings: keep as INT8 (they need higher precision)
        if "tok_emb" in name:
            q, s = quantize_float_tensor(t)
            passthrough[name + "_q"] = q
            passthrough[name + "_s"] = s
            stats["ternary_bytes"] += q.numel() + s.numel() * 2
            continue

        # Large weight matrices: ternary pack
        packed_data, scale, orig_len = pack_ternary(t)
        ternary_packed[name] = packed_data
        ternary_scales[name] = scale
        ternary_shapes[name] = tuple(t.shape)
        ternary_lens[name] = orig_len
        stats["ternary_bytes"] += len(packed_data) + scale.numel() * 2

    return {
        "__quant_format__": "ternary_v1",
        "ternary_packed": ternary_packed,
        "ternary_scales": ternary_scales,
        "ternary_shapes": ternary_shapes,
        "ternary_lens": ternary_lens,
        "passthrough": passthrough,
...
def dequantize_state_dict_ternary(obj: dict[str, object]) -> dict[str, Tensor]:
    """Dequantize ternary-packed state dict back to float tensors."""
    out: dict[str, Tensor] = {}
    for name, data in obj["ternary_packed"].items():
        scale = obj["ternary_scales"][name]
        shape = obj["ternary_shapes"][name]
        orig_len = obj["ternary_lens"][name]
        out[name] = unpack_ternary(data, scale, orig_len, shape).to(torch.bfloat16)
    ...
    for name, t in obj["passthrough"].items():
        if name.endswith("_q"):
            ...
            q = t
            s_key = real_name + "_s"
            if s_key not in obj["passthrough"]:
                # Not an INT8 pair, just pass through
                out[name] = t.detach().cpu()
                continue
            s = obj["passthrough"][s_key]
            if s.ndim > 0 and s.numel() > 1:
                out[real_name] = (q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))).to(torch.bfloat16)
            else:
                out[real_name] = (q.float() * float(s.item())).to(torch.bfloat16)
        elif name.endswith("_s"):
            continue  # handled above with _q
        else:
            orig = t.detach().cpu()
...
class CastedLinear(nn.Linear):
    ...
    _ternary_qat: bool = False

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._qat_active = False
        self._ternary_qat = False

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self._ternary_qat and self.training and self._qat_active:
            # BitNet b1.58 ternary fake quantization via STE
            with torch.no_grad():
                w32 = w.float()
                # AbsMedian scaling per row
                scale = w32.abs().mean(dim=1, keepdim=True).clamp_min(1e-8)
                # Ternary quantization: {-1, 0, +1}
                w_ternary = torch.zeros_like(w32)
                mask_pos = w32 > 0.5 * scale
                mask_neg = w32 < -0.5 * scale
                w_ternary[mask_pos] = scale.expand_as(w32)[mask_pos]
                w_ternary[mask_neg] = -scale.expand_as(w32)[mask_neg]
                w_q = w_ternary.to(x.dtype)
            w = w + (w_q - w).detach()  # STE
        elif self._qat_active and self.training:
            # INT5 QAT fallback (existing code)
            with torch.no_grad():
                w32 = w.float()
                clip_abs = torch.quantile(w32.abs(), INT8_CLIP_Q, dim=1).clamp_min(1e-8)
                scale = clip_abs / INT5_QUANT_RANGE  # INT5
...
def main() -> None:
    ...
    # Log QAT mode
    if USE_TERNARY_QAT:
        log0(f"ternary_qat:enabled bitnet_b158:True")
        log0("qat_mode:ternary_late fake_quant_start:0.5")
    else:
    ...
    # Ternary quantization (primary when USE_TERNARY_QAT=1, otherwise INT5 fallback)
    if USE_TERNARY_QAT:
        ternary_obj, ternary_stats = quantize_state_dict_ternary(base_model.state_dict())
        ternary_buf = io.BytesIO()
        torch.save(ternary_obj, ternary_buf)
        ternary_raw = ternary_buf.getvalue()
        if HAS_ZSTD:
            ternary_blob = zstd.ZstdCompressor(level=22).compress(ternary_raw)
        ...
        if master_process:
            ...
            ternary_file_bytes = os.path.getsize("final_model.ternary.ptz")
            code_bytes = len(code.encode("utf-8"))
            log0(f"Serialized model ternary+{compress_label}: {ternary_file_bytes} bytes")
            log0(f"Total submission size ternary+{compress_label}: {ternary_file_bytes + code_bytes} bytes")
            log0(f"ternary_quant:enabled compression:{compress_label}")
            log0(f"ternary_stats: param_count={ternary_stats['param_count']} ternary_bytes={ternary_stats['ternary_bytes']} baseline_bytes={ternary_stats['baseline_bytes']}")

        # Roundtrip validation using ternary
        if distributed:
            dist.barrier()
        ...
        base_model.load_state_dict(dequantize_state_dict_ternary(rt_state), strict=True)
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(f"final_ternary_{compress_label}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms")
        log0(f"final_ternary_{compress_label}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
        if master_process:
            log0(f"artifact_bytes:{os.path.getsize('final_model.ternary.ptz')}")
    else:
        # INT5 mixed quantization fallback (same as base script)
        int5_obj, int5_stats = quantize_state_dict_int8(base_model.state_dict())
        int5_buf = io.BytesIO()
        torch.save(int5_obj, int5_buf)
        int5_raw = int5_buf.getvalue()
        if HAS_ZSTD:
            int5_blob = zstd.ZstdCompressor(level=22).compress(int5_raw)
            compress_label = "zstd22"
        else:
            int5_blob = zlib.compress(int5_raw, level=9)
            compress_label = "zlib9"
        if master_process:
            ...
            code_bytes = len(code.encode("utf-8"))
            log0(f"Serialized model int5+{compress_label}: {int5_file_bytes} bytes")
            log0(f"Total submission size int5+{compress_label}: {int5_file_bytes + code_bytes} bytes")
            log0(f"int5_quant:enabled compression:{compress_label}")

        # Roundtrip validation using INT8
        if distributed:
            dist.barrier()

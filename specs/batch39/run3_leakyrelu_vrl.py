...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.vrl_gate = nn.Parameter(torch.tensor(-2.0, dtype=torch.float32))  # sigmoid(-2) ~ 0.12
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None, v_residual: Tensor | None = None) -> Tensor:
        ...
        if v_residual is not None:
            g = torch.sigmoid(self.vrl_gate).to(v.dtype)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, v_embed=ve, v_residual=v_residual)
            if i == 0 and self.vrl_enabled:
                v_residual = self.blocks[0].attn.c_v(self.blocks[0].attn_norm(x0))
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, v_embed=ve, v_residual=v_residual)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    if args.vrl_enabled:
        for blk in base_model.blocks:
            scalar_params.append(blk.attn.vrl_gate)

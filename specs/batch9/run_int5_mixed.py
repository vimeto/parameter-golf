...
def quantize_state_dict_mixed(state_dict: dict[str, Tensor]):
    """Mixed quantization: INT5 for MLP weights, INT6 for attention weights, INT8 for embeddings."""
    quantized, scales, dtypes = {}, {}, {}
    passthrough: dict[str, Tensor] = {}
    passthrough_orig_dtypes: dict[str, str] = {}
    qmeta: dict[str, dict[str, object]] = {}
    stats = dict.fromkeys(("param_count", "num_tensors", "num_float_tensors", "num_nonfloat_tensors", "baseline_tensor_bytes", "mixed_payload_bytes"), 0)
    for name, tensor in state_dict.items():
        t = tensor.detach().to("cpu").contiguous()
        stats["param_count"] += int(t.numel())
        stats["num_tensors"] += 1
        ...
        if t.numel() <= INT8_KEEP_FLOAT_MAX_NUMEL:
            ...
            continue
        stats["num_float_tensors"] += 1
        # Determine quantization based on parameter name
        is_embedding = any(pat in name for pat in INT6_FULL_RANGE_PATTERNS)
        is_attn = any(pat in name for pat in ("c_q", "c_k", "c_v"))
        is_mlp = any(pat in name for pat in ("mlp", "fc", "proj")) and not is_attn
        if is_embedding:
            q, s = quantize_float_tensor(t)  # INT8 for embeddings
            bits = 8
        elif is_attn:
            q, s = quantize_float_tensor_int6(t)  # INT6 for attention
            bits = 6
        elif is_mlp:
            q, s = quantize_float_tensor_int5(t)  # INT5 for MLP
            bits = 5
        else:
            q, s = quantize_float_tensor_int5(t)  # Default: INT5
            bits = 5
        qmeta[name] = {"scheme": "per_row" if s.ndim > 0 else "per_tensor", "axis": 0, "bits": bits}
        quantized[name] = q
        scales[name] = s
        dtypes[name] = str(t.dtype).removeprefix("torch.")
        stats["mixed_payload_bytes"] += tensor_nbytes(q) + tensor_nbytes(s)
    obj = {"__quant_format__": "mixed_int5_int6_per_row_v1", "quantized": quantized, "scales": scales, "dtypes": dtypes, "passthrough": passthrough}
    if qmeta: obj["qmeta"] = qmeta
    if passthrough_orig_dtypes: obj["passthrough_orig_dtypes"] = passthrough_orig_dtypes
    return obj, stats
...
def main() -> None:
    ...
    log0("mixed_quant:int5_mlp+int6_attn")
    ...
    # Mixed quantization: INT5 MLP + INT6 attention (primary)
    int6_obj, int6_stats = quantize_state_dict_mixed(base_model.state_dict())
    int6_buf = io.BytesIO()

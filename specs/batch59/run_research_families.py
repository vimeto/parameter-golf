...
class Hyperparameters:
    ...
    research_family = os.environ.get("RESEARCH_FAMILY", "control").strip().lower()
    hnet_chunk_size = int(os.environ.get("HNET_CHUNK_SIZE", 4))
    ut_steps = int(os.environ.get("UT_STEPS", 2))
    random_feature_seed = int(os.environ.get("RANDOM_FEATURE_SEED", 12345))
    strict_feature_check = bool(int(os.environ.get("STRICT_FEATURE_CHECK", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,hnet_mix,state_mix,ut_step_scales,random_feature_gain,random_feature_bias",
    ).split(",")
    if pattern
)
...
def chunk_reduce_mean(x: Tensor, chunk_size: int) -> tuple[Tensor, int]:
    if chunk_size <= 1:
        return x, 0
    pad = (-x.size(1)) % chunk_size
    if pad:
        x = F.pad(x, (0, 0, 0, pad))
    bsz, seqlen, dim = x.shape
    chunks = x.reshape(bsz, seqlen // chunk_size, chunk_size, dim).mean(dim=2)
    return chunks, pad


def chunk_repeat_expand(chunks: Tensor, chunk_size: int, orig_len: int) -> Tensor:
    if chunk_size <= 1:
...
def apply_logit_softcap(logits_proj: Tensor, logit_softcap: float) -> Tensor:
    return logit_softcap * torch.tanh(logits_proj / logit_softcap)


def softcap_cross_entropy_loss(logits_proj: Tensor, targets: Tensor, logit_softcap: float) -> Tensor:
    logits = apply_logit_softcap(logits_proj, logit_softcap)
...
class FastfoodLinear(nn.Module):
    def __init__(self, dim: int, seed: int = 0):
        ...
        self.scale = nn.Parameter(torch.ones(self.padded_dim, dtype=torch.float32))
        self.seed = seed
        self._cached_device: object = None
        self._B: Tensor | None = None
        self._perm: Tensor | None = None
    ...
    def _ensure_buffers(self, device: torch.device) -> None:
        ...
        self._B = (torch.randint(0, 2, (self.padded_dim,), generator=gen) * 2 - 1).float().to(device)
        self._perm = torch.randperm(self.padded_dim, generator=gen).to(device)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x * self._G
        x = fast_walsh_hadamard(x)
        x = x * self.scale.to(dtype=x.dtype)
        if self.dim != self.padded_dim:
            x = x[..., :self.dim]
        return x.to(orig_dtype)


class HierarchicalChunkMixer(nn.Module):
    def __init__(self, dim: int, chunk_size: int, hidden_dim: int):
        super().__init__()
        self.chunk_size = chunk_size
        self.down = CastedLinear(dim, hidden_dim, bias=False) if hidden_dim != dim else None
        self.up = CastedLinear(hidden_dim, dim, bias=False) if hidden_dim != dim else None
    ...
    def forward(self, x: Tensor) -> Tensor:
        chunks, _ = chunk_reduce_mean(x, self.chunk_size)
        if self.down is not None:
            chunks = self.down(chunks)
        chunks = F.silu(chunks)
        if self.up is not None:
            chunks = self.up(chunks)
        return chunk_repeat_expand(chunks, self.chunk_size, x.size(1)) * self.scale.to(dtype=x.dtype)


class ChunkStateMixer(nn.Module):
    def __init__(self, dim: int, chunk_size: int):
        ...
        self.decay_logit = nn.Parameter(torch.full((dim,), 2.0, dtype=torch.float32))
        self.input_gate = nn.Parameter(torch.full((dim,), 0.1, dtype=torch.float32))
        self.scale = nn.Parameter(torch.full((dim,), 0.05, dtype=torch.float32))
        nn.init.orthogonal_(self.in_proj.weight)
        nn.init.orthogonal_(self.out_proj.weight)

    def forward(self, x: Tensor) -> Tensor:
        chunks, _ = chunk_reduce_mean(self.in_proj(x), self.chunk_size)
        decay = torch.sigmoid(self.decay_logit).to(dtype=x.dtype)[None, :]
        gate = torch.sigmoid(self.input_gate).to(dtype=x.dtype)[None, :]
        state = torch.zeros(chunks.size(0), chunks.size(-1), device=x.device, dtype=x.dtype)
        outputs = []
        for ci in range(chunks.size(1)):
            state = decay * state + gate * chunks[:, ci]
            outputs.append(state.unsqueeze(1))
        mixed = torch.cat(outputs, dim=1)
        mixed = self.out_proj(mixed)
        expanded = chunk_repeat_expand(mixed, self.chunk_size, x.size(1))
        return expanded * self.scale.to(dtype=x.dtype)[None, None, :]


class RandomFeatureAdapter(nn.Module):
    def __init__(self, dim: int, rank: int, seed: int):
        super().__init__()
        self.random_map = FastfoodLinear(dim, seed=seed)
        ...
        nn.init.normal_(self.up.weight, std=1.0 / math.sqrt(max(rank, 1)))

    def forward(self, x: Tensor) -> Tensor:
        h = F.silu(self.random_map(x))
        h = self.up(self.down(h))
        return h * self.scale.to(dtype=x.dtype)
...
SOFTCAP_CE_FN = softcap_cross_entropy


class ChunkPoolMixer(nn.Module):
    def __init__(self, dim: int, chunk_size: int):
        super().__init__()
        if chunk_size < 2:
            raise ValueError(f"HNET_CHUNK_SIZE must be >= 2, got {chunk_size}")
        self.chunk_size = chunk_size
...
class CausalStateMixer(nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.state_mix = nn.Parameter(torch.full((dim,), -2.0, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        denom = torch.arange(1, x.size(1) + 1, device=x.device, dtype=torch.float32).view(1, -1, 1)
        running_mean = torch.cumsum(x.float(), dim=1) / denom
        gate = torch.sigmoid(self.state_mix).to(dtype=x.dtype)[None, None, :]
        return gate * (running_mean.to(dtype=x.dtype) - x)
...
class RandomFeatureAdapter(nn.Module):
    def __init__(self, dim: int, seed: int):
        ...
        proj = torch.empty(dim, dim, dtype=torch.float32)
        proj.bernoulli_(0.5, generator=gen).mul_(2.0).sub_(1.0).div_(math.sqrt(dim))
        self.register_buffer("proj", proj, persistent=False)
        self.random_feature_gain = nn.Parameter(torch.full((dim,), -2.0, dtype=torch.float32))
        self.random_feature_bias = nn.Parameter(torch.zeros(dim, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        h = F.linear(x, self.proj.to(dtype=x.dtype))
        h = F.leaky_relu(h, negative_slope=0.25)
        gain = torch.sigmoid(self.random_feature_gain.to(dtype=x.dtype))[None, None, :]
        bias = self.random_feature_bias.to(dtype=x.dtype)[None, None, :]
        return gain * (h + bias)
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_dim: int = 128,
        ve_layers: str = "9,10",
        research_family: str = "control",
        hnet_chunk_size: int = 4,
        ut_steps: int = 0,
        random_feature_seed: int = 12345,
    ):
        super().__init__()
        self._ve_target_dim = num_kv_heads * (model_dim // num_heads)  # kv_dim for value projection
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        if research_family not in {"control", "hnet", "ssm", "ut", "megakernel", "random"}:
            raise ValueError(f"Unsupported RESEARCH_FAMILY={research_family}")
        self.tie_embeddings = tie_embeddings
        self.tied_embed_init_std = tied_embed_init_std
        self.logit_softcap = logit_softcap
        self.mtp_num_heads = mtp_num_heads
        self.mtp_loss_weight = mtp_loss_weight
        self.research_family = research_family
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim) if bigram_vocab_size > 0 else None
        self.smear = SmearGate(model_dim)
        self.hnet_mixer = ChunkPoolMixer(model_dim, hnet_chunk_size) if research_family == "hnet" else None
        self.state_mixer = CausalStateMixer(model_dim) if research_family == "ssm" else None
        self.random_adapter = RandomFeatureAdapter(model_dim, random_feature_seed) if research_family == "random" else None
        self.ut_steps = ut_steps if research_family == "ut" else 0
        self.ut_step_scales = (
            nn.Parameter(torch.zeros(self.ut_steps, dtype=torch.float32))
            if self.ut_steps > 0
            else None
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self.smear(x)
        if self.hnet_mixer is not None:
            x = x + self.hnet_mixer(x)
        if self.state_mixer is not None:
            x = x + self.state_mixer(x)
        if self.random_adapter is not None:
            x = x + self.random_adapter(x)
        x0 = x
        skips: list[Tensor] = []
        ve_cache: dict = {}
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0,
                ...
                v_embed=ve)
        if self.ut_step_scales is not None:
            bi = self.num_layers - 1
            ve = self._get_ve(bi, input_ids, ve_cache)
            for step_idx in range(self.ut_steps):
                step_bias = torch.tanh(self.ut_step_scales[step_idx]).to(dtype=x.dtype)
                refined = self.blocks[bi](
                    x + step_bias,
                    x0,
                    self.qo_bank[bi],
                    self.kv_bank[bi],
        ...
        main_loss = SOFTCAP_CE_FN(logits_proj, targets, self.logit_softcap)
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + SOFTCAP_CE_FN(mtp_logits_proj, mtp_targets, self.logit_softcap)
                mtp_loss_count += 1
            if mtp_loss_count > 0:
...
def main() -> None:
    global SOFTCAP_CE_FN, zeropower_via_newtonschulz5
    ...
    base_model = GPT(
        ...
        hnet_chunk_size=args.hnet_chunk_size,
    ...
    # ---
    megakernel_compiled = False
    if args.research_family == "megakernel":
        if IS_ROCM:
            _inductor_config = __import__("torch._inductor.config", fromlist=["config"])
            _inductor_config.shape_padding = False
            SOFTCAP_CE_FN = torch.compile(softcap_cross_entropy, mode="default", fullgraph=False)
        else:
            SOFTCAP_CE_FN = torch.compile(softcap_cross_entropy, dynamic=False, fullgraph=True)
        megakernel_compiled = True
    if IS_ROCM:
        _inductor_config = __import__("torch._inductor.config", fromlist=["config"])
        _inductor_config.shape_padding = False
    ...
    log0(f"  research_family: {args.research_family}")
    log0(f"  hnet_active: {base_model.hnet_mixer is not None} chunk_size:{args.hnet_chunk_size}")
    log0(f"  state_mixer_active: {base_model.state_mixer is not None}")
    log0(f"  ut_active: {base_model.ut_step_scales is not None} ut_steps:{base_model.ut_steps}")
    log0(f"  random_adapter_active: {base_model.random_adapter is not None} seed:{args.random_feature_seed}")
    log0(f"  megakernel_compiled_loss: {megakernel_compiled}")
    log0("=" * 60)
    if args.strict_feature_check:
        if args.research_family == "hnet" and base_model.hnet_mixer is None:
            raise RuntimeError("RESEARCH_FAMILY=hnet but hnet mixer is inactive")
        if args.research_family == "ssm" and base_model.state_mixer is None:
            raise RuntimeError("RESEARCH_FAMILY=ssm but state mixer is inactive")
        if args.research_family == "ut" and base_model.ut_step_scales is None:
            raise RuntimeError("RESEARCH_FAMILY=ut but recurrent refinement is inactive")
        if args.research_family == "random" and base_model.random_adapter is None:
            raise RuntimeError("RESEARCH_FAMILY=random but random adapter is inactive")
        if args.research_family == "megakernel" and not megakernel_compiled:
            raise RuntimeError("RESEARCH_FAMILY=megakernel but compiled loss path is inactive")
    ...
    handled_param_ids = {id(p) for p in matrix_params}
    handled_param_ids.update(id(p) for p in scalar_params)
    for group in tok_params:
        handled_param_ids.update(id(p) for p in group["params"])
    extra_feature_names: list[str] = []
    for name, p in base_model.named_parameters():
        if id(p) in handled_param_ids or name == "lm_head.weight" or name.startswith("blocks."):
            continue
        if name.startswith(("qo_bank", "kv_bank", "mlp_up_bank", "mlp_down_bank")):
            continue
        if p.ndim >= 2:
    ...
    if extra_feature_names:
        log0(f"extra_feature_params:{','.join(sorted(extra_feature_names))}")
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                if not torch.isfinite(warmup_loss):
                    raise RuntimeError(f"non_finite_warmup_loss step={warmup_step} micro_step={micro_step}")
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(f"non_finite_train_loss step={step} micro_step={micro_step}")
            train_loss += loss.detach()
            (loss * grad_scale).backward()
        train_loss /= grad_accum_steps
        if not torch.isfinite(train_loss):
            raise RuntimeError(f"non_finite_train_loss_reduced step={step}")
    ...
    eval_model = GPT(
        ...
        research_family=args.research_family, hnet_chunk_size=args.hnet_chunk_size,
        ut_steps=args.ut_steps, random_feature_seed=args.random_feature_seed,
    ).to(device).bfloat16()

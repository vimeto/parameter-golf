"""
Kitchen sink script: combines ALL proven leaderboard techniques.
Base: INT5+zstd22+late QAT (batch9)
Added: SmearGate, Mixed INT5/INT6 quant, SWA, Muon weight decay, Orthogonal init, BigramHash

All new features default to disabled (env var = 0) so the script works as a standard model too.
"""
...
class Hyperparameters:
    ...
    # --- Kitchen sink feature flags (all default disabled) ---
    use_smear_gate = bool(int(os.environ.get("USE_SMEAR_GATE", "0")))
    use_mixed_quant = bool(int(os.environ.get("USE_MIXED_QUANT", "0")))
    use_swa = bool(int(os.environ.get("USE_SWA", "0")))
    swa_start_frac = float(os.environ.get("SWA_START_FRAC", "0.4"))
    swa_every = int(os.environ.get("SWA_EVERY", "50"))
    muon_wd = float(os.environ.get("MUON_WD", "0.0"))
    use_ortho_init = bool(int(os.environ.get("USE_ORTHO_INIT", "0")))
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            # Apply weight decay before update (decoupled WD)
...
def _classify_param(name: str) -> str:
    """Classify parameter for mixed quantization routing."""
...
def quantize_state_dict_int5(state_dict: dict[str, Tensor], use_mixed: bool = False):
    """INT5 quantization (default), or mixed INT5-MLP/INT6-attn when use_mixed=True."""
    ...
    for name, tensor in state_dict.items():
        ...
        if use_int8:
            ...
            if cat == "attn":
                ...
                # INT5 for MLP and everything else
                q, s = quantize_float_tensor_int5(t)
                bits_label = 5
        else:
            q, s = quantize_float_tensor_int5(t)
            bits_label = 5
        qmeta[name] = {"scheme": "per_row" if s.ndim > 0 else "per_tensor", "axis": 0, "bits": bits_label}
        quantized[name] = q
        scales[name] = s
        dtypes[name] = str(t.dtype).removeprefix("torch.")
        stats["int5_payload_bytes"] += tensor_nbytes(q) + tensor_nbytes(s)
    quant_label = "int5_mixed56_per_row_v1" if use_mixed else "int5_mixed_per_row_v1"
    obj = {"__quant_format__": quant_label, "quantized": quantized, "scales": scales, "dtypes": dtypes, "passthrough": passthrough}
    ...
    def _init_weights(self) -> None:
        ...
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear):
                ...
                elif self.use_ortho_init and module.weight.ndim == 2 and module.weight.shape[0] >= 64 and module.weight.shape[1] >= 64:
                    nn.init.orthogonal_(module.weight, gain=1.0)
                    # Scale down projection layers to stabilize deep nets
...
def main() -> None:
    ...
    # SmearGate scalar routing
    if base_model.smear is not None:
        scalar_params.append(base_model.smear.gate)

    # BigramHash optimizer routing
    ...
    # Log kitchen sink feature flags
    if args.use_smear_gate:
        log0("smear_gate:enabled")
    if args.use_mixed_quant:
        log0("mixed_quant:enabled (INT5 MLP, INT6 attn)")
    if args.use_swa:
        log0(f"swa:enabled start_frac:{args.swa_start_frac} every:{args.swa_every}")
    if args.muon_wd > 0:
        log0(f"muon_weight_decay:{args.muon_wd}")
    if args.use_ortho_init:
        log0("ortho_init:enabled")
    if args.use_bigram_hash:
        log0(f"bigram_hash_table_size:{args.bigram_vocab_size} bigram_dim:{args.bigram_dim}")
    ...
    # SWA state (accumulated on all ranks to avoid DDP issues; only used at serialization time)
    ...
    while True:
        ...
        # Uses lr_mul scale < swa_start_frac as the trigger (same as leaderboard winner)
        if args.use_swa and scale < args.swa_start_frac and step % args.swa_every == 0:
    ...
    if args.use_swa and swa_state is not None and swa_count > 1:
    ...
    # INT5 (or mixed INT5/INT6) quantization (primary)
    int5_obj, int5_stats = quantize_state_dict_int5(base_model.state_dict(), use_mixed=args.use_mixed_quant)
    int5_buf = io.BytesIO()
    ...
    quant_type_label = "int5_mixed56" if args.use_mixed_quant else "int5"
    if master_process:
        with open("final_model.int5.ptz", "wb") as f:
            f.write(int5_blob)
        int5_file_bytes = os.path.getsize("final_model.int5.ptz")
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model {quant_type_label}+{compress_label}: {int5_file_bytes} bytes")
        log0(f"Total submission size {quant_type_label}+{compress_label}: {int5_file_bytes + code_bytes} bytes")
        log0(f"{quant_type_label}_quant:enabled compression:{compress_label}")
    ...
    torch.cuda.synchronize()
    log0(f"final_{quant_type_label}_{compress_label}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms")
    log0(f"final_{quant_type_label}_{compress_label}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    if master_process:

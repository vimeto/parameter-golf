...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,vrl_gate",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # VRL: Value Residual Learning gate (ResFormer)
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None, v_residual: Tensor | None = None) -> Tensor:
        ...
        if v_embed is not None:
            v = v + v_embed
        # VRL: blend current v with first-layer v_residual
        if self.vrl_enabled and v_residual is not None:
            gate = torch.sigmoid(self.vrl_gate.to(dtype=v.dtype))
            v = (1.0 - gate) * v + gate * v_residual
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, vrl_enabled=vrl_enabled)
...
class GPT(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    vrl_enabled=(vrl_enabled and i > 0),  # VRL on all layers except the first
                )
    ...
    def _compute_v0(self, x0_normed: Tensor) -> Tensor:
        """Compute v0 from the first block's value projection for VRL."""
        return self.blocks[0].attn.c_v(x0_normed)
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # VRL: compute v0 from first block's value projection on normalized x0
        v0: Tensor | None = None
        if self.vrl_enabled:
            x0_normed = self.blocks[0].attn_norm(x0) * self.blocks[0].ln_scale_factor
            v0 = self._compute_v0(x0_normed)
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x = self.blocks[i](x, x0, v_embed=ve, v_residual=v0)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, v_embed=ve, v_residual=v0)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    # VRL: add vrl_gate params to scalar optimizer
    if args.vrl_enabled:
        for block in base_model.blocks:
            if hasattr(block.attn, 'vrl_gate'):

...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.fc is None:
            # SwiGLU activation
            return self.proj(F.leaky_relu(self.w_gate(x), 0.5) * self.w_up(x))
        else:
...
def main() -> None:
    ...
    hidden = int(args.mlp_mult * args.model_dim)
    if USE_SWIGLU:
        log0(f"activation:leaky_swiglu hidden={hidden}")
    else:
        log0(f"activation:relu_squared hidden={hidden}")

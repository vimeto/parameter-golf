...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_delta_fn=None, v_delta_fn=None) -> Tensor:
        ...
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * self.attn(n, q_delta=qd, v_delta=vd)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if lora is not None:
                q_delta_fn = lambda n, i=i: lora.q_loras[i](n)
                v_delta_fn = lambda n, i=i: lora.v_loras[i](n)
            else:
                q_delta_fn = v_delta_fn = None
            x = self.blocks[i](x, x0, q_delta_fn, v_delta_fn)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if lora is not None:
                q_delta_fn = lambda n, bi=bi: lora.q_loras[bi](n)
                v_delta_fn = lambda n, bi=bi: lora.v_loras[bi](n)
            else:
                q_delta_fn = v_delta_fn = None
            x = self.blocks[bi](x, x0, q_delta_fn, v_delta_fn)
        ...
        if lora is not None:
            logits_proj = logits_proj + lora.lm_head_lora(x_final).reshape(-1, logits_proj.size(-1))
        ...
        if lora is not None:
            ...
            return F.cross_entropy(
                logits.float().reshape(-1, V), targets, reduction="none"
            ).reshape(bsz, sl)
...
def main() -> None:
    ...
    if not IS_ROCM:
        ...
        pass  # ROCm uses default SDP dispatch

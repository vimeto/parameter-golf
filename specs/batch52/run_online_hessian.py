...
def main() -> None:
    ...
    log0(f"  online_hessian: True (hooks on attn/mlp modules, 25-step interval)")
    ...
    if args.warmup_steps > 0:
        ...
        zero_grad_all()
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    # --- Online Hessian accumulation setup ---
    ...
    model_dim = args.model_dim
    mlp_dim = int(args.mlp_mult * model_dim)
    # Initialize Hessian buffers for Q/K/V (from attn input) and MLP up (from mlp input)
    for i in range(args.num_layers):
        _oh_bufs[f'blocks.{i}.attn.c_q.weight'] = torch.zeros(model_dim, model_dim, dtype=torch.float32, device='cpu')
        _oh_bufs[f'blocks.{i}.attn.c_k.weight'] = torch.zeros(model_dim, model_dim, dtype=torch.float32, device='cpu')
        _oh_bufs[f'blocks.{i}.attn.c_v.weight'] = torch.zeros(model_dim, model_dim, dtype=torch.float32, device='cpu')
        _oh_bufs[f'blocks.{i}.mlp.fc.weight'] = torch.zeros(model_dim, model_dim, dtype=torch.float32, device='cpu')
    # Register forward hooks on attn and mlp modules to capture inputs
    _oh_hooks = []
    ...
    log0(f"online_hessian:setup hooks={len(_oh_hooks)} buffers={len(_oh_bufs)} every={ONLINE_HESSIAN_EVERY}")
    ...
    # Finalize online Hessians: remove hooks and normalize
    for h in _oh_hooks:
        h.remove()
    _oh_hooks.clear()
    ...
    if FULL_GPTQ:
        ...
        for k, v in post_hessians.items():
            if k in _oh_bufs and online_hessian_count > 0:
                # Online Hessian available -- use it (accumulated over many more steps)
                hessians[k] = _oh_bufs[k]
                log0(f"gptq:using online hessian for {k} (n={online_hessian_count})")
            else:
                # Use post-training Hessian (out_proj, mlp_down, CastedLinear)
                hessians[k] = v
        log0(f"gptq:merged hessians: {sum(1 for k in hessians if k in _oh_bufs and online_hessian_count > 0)} online, "
             f"{sum(1 for k in hessians if k not in _oh_bufs or online_hessian_count == 0)} post-training")

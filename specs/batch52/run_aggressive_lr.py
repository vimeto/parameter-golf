...
class Hyperparameters:
    ...
    batch_warmup_tokens_start = int(os.environ.get("BATCH_WARMUP_TOKENS_START", 131_072))
    batch_warmup_tokens_mid = int(os.environ.get("BATCH_WARMUP_TOKENS_MID", 262_144))
    batch_warmup_phase1_frac = float(os.environ.get("BATCH_WARMUP_PHASE1_FRAC", 0.15))
    batch_warmup_phase2_frac = float(os.environ.get("BATCH_WARMUP_PHASE2_FRAC", 0.35))
...
def main() -> None:
    ...
    log0(f"  batch_warmup: {args.batch_warmup_enabled} ({args.batch_warmup_tokens_start} -> {args.batch_warmup_tokens_mid} -> {args.train_batch_tokens})")
    log0(f"  matrix_lr: {args.matrix_lr} (aggressive)")
    log0("=" * 60)
    ...
    while True:
        ...
        # Batch size warmup: 120K -> 260K -> full
        if args.batch_warmup_enabled and max_wallclock_ms is not None:
            progress = elapsed_ms / max_wallclock_ms
            if progress < args.batch_warmup_phase1_frac:
                current_batch_tokens = args.batch_warmup_tokens_start
            elif progress < args.batch_warmup_phase2_frac:
                current_batch_tokens = args.batch_warmup_tokens_mid
            else:

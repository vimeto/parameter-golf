...
class Hyperparameters:
    ...
    recur_layers = os.environ.get("RECUR_LAYERS", "4,5")  # physical layers to recur (0-indexed)
    recur_loops = int(os.environ.get("RECUR_LOOPS", 2))  # how many times each recurred layer runs
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,recur_extra_scalars",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None,
                override_attn_scale: Tensor | None = None, override_mlp_scale: Tensor | None = None, override_resid_mix: Tensor | None = None) -> Tensor:
        attn_s = override_attn_scale if override_attn_scale is not None else self.attn_scale
        mlp_s = override_mlp_scale if override_mlp_scale is not None else self.mlp_scale
        rmix = override_resid_mix if override_resid_mix is not None else self.resid_mix
        mix = rmix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed)
        x_out = x_in + attn_s.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + mlp_s.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor, up_w, down_w)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Build virtual-to-physical layer mapping for recurrence
        recur_set = set(int(x) for x in recur_layers.split(",") if x.strip())
        self.recur_v2p: list[int] = []  # virtual-to-physical mapping
        self.recur_pass_idx: list[int] = []  # which pass (0=first, 1=second, etc.)
        for i in range(num_layers):
            for loop in range(recur_loops if i in recur_set else 1):
                self.recur_v2p.append(i)
                self.recur_pass_idx.append(loop)
        self.num_virtual_layers = len(self.recur_v2p)
        # U-Net skip connections based on virtual layer count
        self.num_encoder_layers = self.num_virtual_layers // 2
        self.num_decoder_layers = self.num_virtual_layers - self.num_encoder_layers
        ...
        # Extra per-virtual-block scalars for recurred passes (pass_idx > 0)
        self.recur_extra_scalars = nn.ParameterDict()
        extra_idx = 0
        for vi in range(self.num_virtual_layers):
            if self.recur_pass_idx[vi] > 0:
                self.recur_extra_scalars[f"attn_scale_{extra_idx}"] = nn.Parameter(torch.ones(model_dim, dtype=torch.float32))
                self.recur_extra_scalars[f"mlp_scale_{extra_idx}"] = nn.Parameter(torch.ones(model_dim, dtype=torch.float32))
                self.recur_extra_scalars[f"resid_mix_{extra_idx}"] = nn.Parameter(torch.stack([torch.ones(model_dim), torch.zeros(model_dim)]).float())
                extra_idx += 1
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        n_virt = self.num_virtual_layers
        ...
        extra_idx = 0
        for vi in range(n_virt):
            pi = self.recur_v2p[vi]
            pass_i = self.recur_pass_idx[vi]
            ve = self._get_ve(pi, input_ids, ve_cache)
            # Get the right scalars for this virtual layer
            if pass_i > 0:
                override_attn = self.recur_extra_scalars[f"attn_scale_{extra_idx}"]
                override_mlp = self.recur_extra_scalars[f"mlp_scale_{extra_idx}"]
                override_rmix = self.recur_extra_scalars[f"resid_mix_{extra_idx}"]
                extra_idx += 1
            else:
                override_attn = None
                override_mlp = None
            ...
            if vi < num_enc:
                ...
                x = self.blocks[pi](x, x0,
                    ...
                    v_embed=ve,
                    override_attn_scale=override_attn, override_mlp_scale=override_mlp, override_resid_mix=override_rmix)
                skips.append(x)
            else:
                # Decoder phase
                dec_i = vi - num_enc
                if dec_i < self.num_skip_weights and skips:
    ...
    for block_idx, block in enumerate(model.blocks):
        ...
        def make_wrapped_forward(blk, orig_fwd, qk, kk, vk, ok, upk, dnk):
            def wrapped_forward(x, x0, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=None,
                                override_attn_scale=None, override_mlp_scale=None, override_resid_mix=None):
                # Capture attn_norm output (input to Q, K, V)
                rmix = override_resid_mix if override_resid_mix is not None else blk.resid_mix
                attn_s = override_attn_scale if override_attn_scale is not None else blk.attn_scale
                mlp_s = override_mlp_scale if override_mlp_scale is not None else blk.mlp_scale
                mix = rmix.to(dtype=x.dtype)
                x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
                ...
                result = orig_fwd(x, x0, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed,
                                  override_attn_scale=override_attn_scale, override_mlp_scale=override_mlp_scale,
                                  override_resid_mix=override_resid_mix)
...
def main() -> None:
    ...
    log0(f"  recurrence: layers={args.recur_layers} loops={args.recur_loops} virtual_layers={base_model.num_virtual_layers}")
    log0(f"  recur_v2p: {base_model.recur_v2p}")
    log0(f"  recur_extra_scalars: {len(base_model.recur_extra_scalars)} params")
    log0("=" * 60)
    ...
    # Add recurrence extra scalars to optimizer
    for key, p in base_model.recur_extra_scalars.items():
    ...
    eval_model = GPT(
        ...
        recur_layers=args.recur_layers, recur_loops=args.recur_loops,
    ).to(device).bfloat16()

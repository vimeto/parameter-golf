...
def main() -> None:
    ...
    log0(f"  adam_alternation: True (Adam steps on odd iterations only)")
    log0("=" * 60)
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            # 3-phase warmup step (Adam alternates on odd steps)
            do_adam_warmup = (warmup_step % 2 == 1)
            ...
            if do_adam_warmup:
                ...
                if optimizer_head is not None:
                    optimizer_head.step()
            optimizer_muon.step()
            # Only zero Muon grads on even steps; zero all on odd steps
            if do_adam_warmup:
                zero_grad_all()
            else:
                # Zero only Muon bank grads, accumulate Adam grads for next step
                for p in [base_model.qo_bank, base_model.kv_bank, base_model.mlp_up_bank, base_model.mlp_down_bank]:
                    if p.grad is not None:
                        p.grad = None
    ...
    while True:
        ...
        # On even steps, zero all grads; on odd steps, Adam grads already accumulated
        if step % 2 == 0:
            zero_grad_all()
        ...
        # === 3-phase overlapped optimizer step (Adam alternates on odd steps) ===
        do_adam = (step % 2 == 1)

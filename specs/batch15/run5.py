...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if self.recurrent_blocks > 0:
            ...
            # Recurrent core: blocks 1..recurrent_blocks looped recurrent_loops times
            for loop in range(self.recurrent_loops):
...
def main() -> None:
    ...
    if args.recurrent_blocks > 0:
        log0(f"recurrence:blocks={args.recurrent_blocks} loops={args.recurrent_loops} effective_depth={base_model.effective_depth} stored_layers={len(list(base_model.blocks))} lora_rank={args.recurrent_lora_rank}")
    else:

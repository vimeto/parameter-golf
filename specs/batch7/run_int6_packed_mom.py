...
def pack_int6_to_bytes(tensor_int8: Tensor) -> tuple[bytes, int]:
    """Pack INT6 values (stored in int8, range [-32,31]) into 6 bits each.
    Every 4 values -> 3 bytes. Pad to multiple of 4 if needed."""
    flat = tensor_int8.reshape(-1).numpy().astype(np.int8)
    n = len(flat)
    # Pad to multiple of 4
    pad = (4 - n % 4) % 4
    if pad:
        flat = np.concatenate([flat, np.zeros(pad, dtype=np.int8)])
    # Convert to unsigned 6-bit (0-63): add 32 to shift from [-32,31] to [0,63]
    u6 = (flat.astype(np.int16) + 32).astype(np.uint8)
    # Pack 4 values into 3 bytes: AAAAAA|BB BBBB|CCCC CC|DDDDDD
    a, b, c, d = u6[0::4], u6[1::4], u6[2::4], u6[3::4]
    b0 = (a << 2) | (b >> 4)
    b1 = ((b & 0x0F) << 4) | (c >> 2)
    b2 = ((c & 0x03) << 6) | d
    packed = np.stack([b0, b1, b2], axis=1).reshape(-1)
    return bytes(packed), n  # return original length for unpacking

def unpack_bytes_to_int6(data: bytes, orig_len: int, shape: tuple) -> Tensor:
    """Unpack 6-bit packed bytes back to INT6 values in int8 tensor."""
    ...
    c = ((b1 & 0x0F) << 2) | (b2 >> 6)
    d = b2 & 0x3F
    unpacked = np.stack([a, b, c, d], axis=1).reshape(-1)[:orig_len]
    # Convert back from unsigned [0,63] to signed [-32,31]
    values = unpacked.astype(np.int16) - 32
...
def quantize_state_dict_int6(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        qmeta[name] = {"scheme": "per_row" if s.ndim > 0 else "per_tensor", "axis": 0, "bits": 8 if use_int8 else 6}
        quantized[name] = q
        scales[name] = s
        dtypes[name] = str(t.dtype).removeprefix("torch.")
        stats["int6_payload_bytes"] += tensor_nbytes(q) + tensor_nbytes(s)
    obj = {"__quant_format__": "int6_mixed_per_row_v1", "quantized": quantized, "scales": scales, "dtypes": dtypes, "passthrough": passthrough}
...
def quantize_state_dict_int6_packed(state_dict: dict[str, Tensor]):
    quantized_packed: dict[str, bytes] = {}
    ...
    passthrough: dict[str, Tensor] = {}
    passthrough_orig_dtypes: dict[str, str] = {}
    stats = dict.fromkeys(("param_count", "num_tensors", "num_float_tensors", "num_nonfloat_tensors", "baseline_tensor_bytes", "packed_payload_bytes"), 0)
    for name, tensor in state_dict.items():
        t = tensor.detach().to("cpu").contiguous()
        stats["param_count"] += int(t.numel())
        stats["num_tensors"] += 1
        ...
        if use_int8:
            ...
            stats["packed_payload_bytes"] += tensor_nbytes(q) + tensor_nbytes(s)
        else:
            q, s = quantize_float_tensor_int6(t)
            packed_data, orig_len = pack_int6_to_bytes(q)
            quantized_packed[name] = packed_data
            shapes[name] = tuple(q.shape)
            orig_lens[name] = orig_len
            stats["packed_payload_bytes"] += len(packed_data) + tensor_nbytes(s)
        scales[name] = s
        dtypes[name] = str(t.dtype).removeprefix("torch.")
    obj = {
...
def dequantize_state_dict_int6_packed(obj: dict[str, object]) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    for name, packed_data in obj["quantized_packed"].items():
        dtype = getattr(torch, obj["dtypes"][name])
        s = obj["scales"][name].to(dtype=torch.float32)
        shape = obj["shapes"][name]
        orig_len = obj["orig_lens"][name]
        q = unpack_bytes_to_int6(packed_data, orig_len, shape)
        if s.ndim > 0:
            out[name] = (q.float() * s.view(q.shape[0], *([1] * (q.ndim - 1)))).to(dtype=dtype).contiguous()
        else:
            out[name] = (q.float() * float(s.item())).to(dtype=dtype).contiguous()
    for name, q in obj.get("quantized_int8", {}).items():
        dtype = getattr(torch, obj["dtypes"][name])
        s = obj["scales"][name].to(dtype=torch.float32)
        if s.ndim > 0:
            out[name] = (q.float() * s.view(q.shape[0], *([1] * (q.ndim - 1)))).to(dtype=dtype).contiguous()
...
def main() -> None:
    ...
    # INT6 bit-packed mixed quantization (primary)
    int6_obj, int6_stats = quantize_state_dict_int6_packed(base_model.state_dict())
    int6_buf = io.BytesIO()
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int6_packed+{compress_label}: {int6_file_bytes} bytes")
        log0(f"Total submission size int6_packed+{compress_label}: {int6_file_bytes + code_bytes} bytes")
        log0(f"int6_packed:enabled compression:{compress_label}")
    ...
    base_model.load_state_dict(dequantize_state_dict_int6_packed(rt_state), strict=True)
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    log0(f"final_int6_{compress_label}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms")
    log0(f"final_int6_{compress_label}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    if master_process:
        log0(f"artifact_bytes:{os.path.getsize('final_model.int6.ptz')}")

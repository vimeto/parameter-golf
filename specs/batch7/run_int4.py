...
def quantize_float_tensor_int4(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        clipped = torch.maximum(torch.minimum(t32, clip_abs[:, None]), -clip_abs[:, None])
        scale = (clip_abs / INT4_QUANT_RANGE).clamp_min(1.0 / INT4_QUANT_RANGE)
        q = torch.clamp(torch.round(clipped / scale[:, None]), -8, INT4_QUANT_RANGE).to(torch.int8).contiguous()
        return q, scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()
    clip_abs = float(torch.quantile(t32.abs().flatten(), INT8_CLIP_Q).item()) if t32.numel() else 0.0
    scale = torch.tensor(clip_abs / INT4_QUANT_RANGE if clip_abs > 0 else 1.0, dtype=torch.float32)
    q = torch.clamp(torch.round(torch.clamp(t32, -clip_abs, clip_abs) / scale), -8, INT4_QUANT_RANGE).to(torch.int8).contiguous()
    return q, scale
...
def quantize_state_dict_int6(state_dict: dict[str, Tensor]):
    ...
    passthrough_orig_dtypes: dict[str, str] = {}
    qmeta: dict[str, dict[str, object]] = {}
    stats = dict.fromkeys(("param_count", "num_tensors", "num_float_tensors", "num_nonfloat_tensors", "baseline_tensor_bytes", "int4_payload_bytes"), 0)
    for name, tensor in state_dict.items():
        t = tensor.detach().to("cpu").contiguous()
        stats["param_count"] += int(t.numel())
        stats["num_tensors"] += 1
        ...
        qmeta[name] = {"scheme": "per_row" if s.ndim > 0 else "per_tensor", "axis": 0, "bits": 8 if use_int8 else 4}
        quantized[name] = q
        scales[name] = s
        dtypes[name] = str(t.dtype).removeprefix("torch.")
        stats["int4_payload_bytes"] += tensor_nbytes(q) + tensor_nbytes(s)
    obj = {"__quant_format__": "int4_mixed_per_row_v1", "quantized": quantized, "scales": scales, "dtypes": dtypes, "passthrough": passthrough}
...
def main() -> None:
    ...
    # INT4 mixed quantization (primary)
    ...
    if master_process:
        with open("final_model.int4.ptz", "wb") as f:
            f.write(int6_blob)
        int6_file_bytes = os.path.getsize("final_model.int4.ptz")
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int4+{compress_label}: {int6_file_bytes} bytes")
        log0(f"Total submission size int4+{compress_label}: {int6_file_bytes + code_bytes} bytes")
        log0(f"int4_quant:enabled compression:{compress_label}")
    ...
    torch.cuda.synchronize()
    log0(f"final_int4_{compress_label}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms")
    log0(f"final_int4_{compress_label}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    if master_process:
        log0(f"artifact_bytes:{os.path.getsize('final_model.int4.ptz')}")

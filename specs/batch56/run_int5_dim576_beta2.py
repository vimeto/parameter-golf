...
def gptq_quantize_layer(weight: Tensor, hessian: Tensor = None, clip_range: int = 15,
...
def main() -> None:
    ...
    log0(f"  model_dim: {args.model_dim} (wider)")
    log0(f"  quantization: INT5 for banks (5-bit, range [-15,15]), INT8 for embeddings")
    ...
    torch.cuda.synchronize()
    log0(
        f"final_int5_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int5_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_int5_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    ...
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int5_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_int5_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")

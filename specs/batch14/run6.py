...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if self.recurrent_blocks > 0:
            ...
            lora_idx = 1  # track effective layer index for LoRA
            for loop in range(self.recurrent_loops):
                for b in range(self.recurrent_blocks):
                    ...
                    # LoRA: use block index within stored blocks (the same block gets same LoRA)
                    qd = lora.q_loras[1 + b] if lora else None
                    vd = lora.v_loras[1 + b] if lora else None
                    x = block(x, x0, qd, vd)

...
class Hyperparameters:
    ...
    use_blockdiag_mlp = bool(int(os.environ.get("USE_BLOCKDIAG_MLP", "0")))
    mlp_num_blocks = int(os.environ.get("MLP_NUM_BLOCKS", 4))
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    # Handle 3D block-diagonal weights by reshaping to 2D
                    orig_shape = g.shape
                    if g.ndim == 3:
                        g = g.reshape(-1, g.size(-1))  # (num_blocks*block_out, block_in)
...
class BlockDiagLinear(nn.Module):
    """Block-diagonal linear layer. num_blocks splits input and output into blocks."""
    def __init__(self, in_features: int, out_features: int, num_blocks: int, bias: bool = False):
        super().__init__()
        ...
        # Shape: (num_blocks, block_out, block_in)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Reshape: (B*seq, num_blocks, block_in)
        x_flat = x.reshape(-1, self.num_blocks, self.block_in).to(self.weight.dtype)
        # For each block: x_block @ W_block^T
        # weight: (num_blocks, block_out, block_in)
        # Transpose to (num_blocks, block_in, block_out)
        w = self.weight
        if self._qat_active and self.training:
            with torch.no_grad():
                w32 = w.float()
                # Per-row quantization: reshape to 2D, quantize, reshape back
                w2d = w32.reshape(-1, self.block_in)
                ...
                w_q = w_q.reshape(self.num_blocks, self.block_out, self.block_in).to(x.dtype)
            w = w + (w_q - w).detach()  # STE
        wt = w.transpose(1, 2)
        # Batched matmul: need (num_blocks, B*seq, block_in) @ (num_blocks, block_in, block_out)
        x_perm = x_flat.permute(1, 0, 2)  # (num_blocks, B*seq, block_in)
        out = torch.bmm(x_perm, wt)  # (num_blocks, B*seq, block_out)
        out = out.permute(1, 0, 2)  # (B*seq, num_blocks, block_out)
...
class BlockDiagMixedLinear(nn.Module):
    """Block-diagonal linear + cross-block mixing via a learned mixing matrix."""
    def __init__(self, in_features: int, out_features: int, num_blocks: int, bias: bool = False):
        super().__init__()
        self.block_linear = BlockDiagLinear(in_features, out_features, num_blocks, bias)
        # Cheap cross-block mixing: a small dense layer on the block dimension
        self.mix = nn.Parameter(torch.eye(num_blocks) + 0.01 * torch.randn(num_blocks, num_blocks))
        self._qat_active = False

    @property
    def weight(self):
        return self.block_linear.weight

    def forward(self, x: Tensor) -> Tensor:
        self.block_linear._qat_active = self._qat_active
        out = self.block_linear(x)
        # Apply cross-block mixing
        orig_shape = out.shape
        block_out = self.block_linear.block_out
        num_blocks = self.block_linear.num_blocks
        out = out.view(*orig_shape[:-1], num_blocks, block_out)
        # Mix across blocks: mix is (num_blocks, num_blocks)
        # out: (..., num_blocks, block_out), mix: (num_blocks, num_blocks)
        # output: (..., num_blocks, block_out) where each output block is a weighted sum of input blocks
        mix = self.mix.to(out.dtype)
        out_flat = out.reshape(-1, num_blocks, block_out)  # (B*seq, num_blocks, block_out)
        # (B*seq, num_blocks, block_out) -> permute -> (B*seq, block_out, num_blocks) @ mix^T -> permute back
        out_flat = out_flat.permute(0, 2, 1)  # (B*seq, block_out, num_blocks)
        out_flat = torch.matmul(out_flat, mix.T)  # (B*seq, block_out, num_blocks)
        out_flat = out_flat.permute(0, 2, 1)  # (B*seq, num_blocks, block_out)
        return out_flat.reshape(orig_shape).to(x.dtype)


class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, use_blockdiag: bool = False, num_blocks: int = 4):
        super().__init__()
        hidden = mlp_mult * dim
        if use_blockdiag:
            self.fc = BlockDiagMixedLinear(dim, hidden, num_blocks)
            self.proj = BlockDiagMixedLinear(hidden, dim, num_blocks)
        else:
            self.fc = CastedLinear(dim, hidden, bias=False)
            self.proj = CastedLinear(hidden, dim, bias=False)
        self.proj._zero_init = True
        self.use_blockdiag = use_blockdiag
...
class Block(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        use_blockdiag_mlp: bool = False,
        mlp_num_blocks: int = 4,
    ):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = MLP(dim, mlp_mult, use_blockdiag=use_blockdiag_mlp, num_blocks=mlp_num_blocks)
...
class GPT(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    use_blockdiag_mlp=use_blockdiag_mlp,
    ...
    def _init_weights(self) -> None:
        ...
        for module in self.modules():
            ...
            if isinstance(module, BlockDiagMixedLinear) and getattr(module, "_zero_init", False):
                nn.init.zeros_(module.block_linear.weight)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        use_blockdiag_mlp=args.use_blockdiag_mlp,
        mlp_num_blocks=args.mlp_num_blocks,
    ).to(device).bfloat16()
    for module in base_model.modules():
        ...
        if isinstance(module, BlockDiagMixedLinear):
            module.mix.data = module.mix.data.float()
    ...
    matrix_params = [
        ...
        and 'mix' not in name  # exclude cross-block mixing matrices (small, not suited for Muon)
    ]
    scalar_params = [
        p
        for name, p in block_named_params
        if (p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
            or 'mix' in name)  # cross-block mixing matrices go to Adam
        and not (p.ndim >= 2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
                 and 'mix' not in name)  # exclude what's already in matrix_params
    ]
    ...
    if args.use_blockdiag_mlp:
        log0(f"blockdiag_mlp:enabled num_blocks:{args.mlp_num_blocks} params_reduction:{args.mlp_num_blocks}x")
    ...
    while True:
        ...
        if frac_done >= 0.5:
            for m in base_model.modules():
                ...
                if isinstance(m, BlockDiagMixedLinear) and not m._qat_active:
                    m._qat_active = True
                    log0(f"late_qat_switchover_blockdiag_step:{step}")
        zero_grad_all()

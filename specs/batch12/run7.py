    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._qat_active and self.training:
            with torch.no_grad():
                ...
                scale = clip_abs / INT6_QUANT_RANGE  # Use INT6_QUANT_RANGE = 31
...
def main() -> None:
    ...
    log0("qat_mode:int6_late fake_quant_start:0.5")
    ...
    # INT6 mixed quantization (primary)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int6+{compress_label}: {int6_file_bytes} bytes")
        log0(f"Total submission size int6+{compress_label}: {int6_file_bytes + code_bytes} bytes")
        log0(f"int6_quant:enabled compression:{compress_label}")

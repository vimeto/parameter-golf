...
class Hyperparameters:
    ...
    batch_warmup_small = int(os.environ.get("BATCH_WARMUP_SMALL", 131_072))
    batch_warmup_switch_frac = float(os.environ.get("BATCH_WARMUP_SWITCH_FRAC", 0.15))
...
def main() -> None:
    ...
    # Online Hessian setup
    ONLINE_HESSIAN_EVERY = 25
    online_hessian_count = 0
    _oh_hessians = None
    log0(f"online_hessian:mode=separate_forward_pass every={ONLINE_HESSIAN_EVERY} steps")
    log0("=" * 60)
    ...
    log0(f"  batch_warmup: {args.batch_warmup_enabled} small={args.batch_warmup_small} switch_frac={args.batch_warmup_switch_frac}")
    log0(f"  online_hessian: True (every {ONLINE_HESSIAN_EVERY} steps, after scale < 0.8)")
    log0("=" * 60)
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and step > 100 and not CastedLinear._qat_enabled:
        ...
        if args.batch_warmup_enabled and max_wallclock_ms is not None:
            ...
            current_batch_tokens = args.train_batch_tokens
        # Online Hessian: every 25 steps after 20% of training
        _oh_accumulate = (step % ONLINE_HESSIAN_EVERY == 0 and scale < 0.8)
        if _oh_accumulate:
            base_model.eval()
            step_hessians = collect_hessians(
                base_model, train_loader, args, device, 1,

...
- Fastfood MLP: structured linear layers with O(d) params via Walsh-Hadamard + seed-based random projections
"""
...
class Hyperparameters:
    ...
    use_fastfood_mlp = bool(int(os.environ.get("USE_FASTFOOD_MLP", "0")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,fastfood_mlp",
    ).split(",")
    if pattern
)
...
def fast_walsh_hadamard(x: Tensor) -> Tensor:
    ...
    n = x.shape[-1]
    orig_shape = x.shape
    # Flatten all but last dim for batched processing
    x = x.reshape(-1, n)
    h = 1
    while h < n:
        ...
        x = x.view(-1, n)
        h *= 2
    return (x / (n ** 0.5)).reshape(orig_shape)


class FastfoodLinear(nn.Module):
    """Fastfood transform: structured linear layer with O(d) learnable params instead of O(d^2).

    Computes y = S * H * G * Pi * H * B * x where:
    - B = diagonal of random +/-1 (regenerated from seed, NOT stored)
    - H = Walsh-Hadamard transform (no params, O(n log n))
    - Pi = random permutation (regenerated from seed, NOT stored)
    - G = diagonal of random Gaussians (regenerated from seed, NOT stored)
    - S = diagonal scaling (LEARNED, d params)

    Only self.scale is stored in the state dict. B, Pi, G are regenerated from seed at runtime.
    """

    def __init__(self, dim: int, seed: int = 0):
        super().__init__()
        self.dim = dim
        self.padded_dim = 1 << (dim - 1).bit_length()  # next power of 2
        self.scale = nn.Parameter(torch.ones(self.padded_dim))  # S: learned diagonal
        self.seed = seed
        # Cache for regenerated random components
    ...
    def _ensure_buffers(self, device: torch.device) -> None:
        """Regenerate B, perm, G from seed on the target device. Cached until device changes."""
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        orig_dtype = x.dtype
        x = x.float()
        # Pad if dim is not a power of 2
        if self.dim != self.padded_dim:
            x = F.pad(x, (0, self.padded_dim - self.dim))
        # Fastfood: S * H * G * Pi * H * B * x
        x = x * self._B                        # B: random +/-1 diagonal
        x = fast_walsh_hadamard(x)              # H: Walsh-Hadamard
        x = x[..., self._perm]                  # Pi: random permutation
        x = x * self._G                         # G: random Gaussian diagonal
        x = fast_walsh_hadamard(x)              # H: Walsh-Hadamard
        x = x * self.scale                      # S: learned scaling
        # Unpad if needed
        if self.dim != self.padded_dim:
            x = x[..., :self.dim]
...
class FastfoodMLP(nn.Module):
    """MLP using Fastfood transforms for structured mixing with O(d) params per transform layer.

    Replaces the standard dense MLP (2 * dim * hidden params) with two Fastfood
    transforms operating in model dimension (4 * dim params total for the Fastfood part).
    ...
    def __init__(self, dim: int, hidden_mult: int = 2, seed_base: int = 42):
        super().__init__()
        self.dim = dim
        self.hidden_mult = hidden_mult
        # Fastfood mixing layers (O(d) params each instead of O(d*hidden))
        self.ff1 = FastfoodLinear(dim, seed=seed_base)
        self.ff2 = FastfoodLinear(dim, seed=seed_base + 1)
        # Small learnable scales
        self.gate_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.out_scale = nn.Parameter(torch.full((dim,), 0.1, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        h = self.ff1(x * self.gate_scale.to(dtype=x.dtype))
        h = F.relu(h).square()
        h = self.ff2(h)
        return h * self.out_scale.to(dtype=x.dtype)
...
class Block(nn.Module):
    def __init__(
        ...
        if use_fastfood_mlp:
            self.mlp = FastfoodMLP(dim, hidden_mult=mlp_mult, seed_base=layer_idx * 100 + 42)
        else:
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    use_fastfood_mlp=use_fastfood_mlp,
                    layer_idx=i,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        use_fastfood_mlp=args.use_fastfood_mlp,
    ).to(device).bfloat16()
    ...
    if args.use_fastfood_mlp:
        log0(f"fastfood_mlp:enabled dim_params_per_layer:{4 * args.model_dim}")

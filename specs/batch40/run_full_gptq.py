...
def gptq_quantize_layer(W: Tensor, H: Tensor, clip_range: int = 31,
    ...
    H: (in_features, in_features) Hessian matrix (X^T X from calibration)
    Returns: (quantized_int8, per_row_scale)
    """
    W = W.float().clone()
    rows, cols = W.shape
    ...
    # Compute per-row scale using best-of-5 percentile search (same as GPTQ-lite)
    best_s = None
    ...
    for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
        ...
        # Quick MSE estimate with naive quantization
    ...
    for col_start in range(0, cols, block_size):
        ...
        # Working copy of the weight block (will be modified by compensation)
        W_block = W[:, col_start:col_end].clone()

        # Get Hessian inverse block for this block of columns
        ...
        for j in range(block_cols):
            ...
            s_f = scale.float()  # (rows,)
            q = torch.clamp(torch.round(w / s_f), -clip_range, clip_range)
            Q[:, col] = q

            # Quantization error (in weight space, not quantized space)
            err = w - q * s_f  # (rows,)

            # Compensate remaining columns in this block using Hessian inverse
            if j + 1 < block_cols:
                hinv_jj = Hinv_block[j, j].clamp_min(1e-10)
                # err: (rows,), Hinv_block[j, j+1:]: (remaining_cols,)
                # compensation: (rows, remaining_cols)
                compensation = err.unsqueeze(1) * Hinv_block[j, j + 1:].unsqueeze(0) / hinv_jj
                W_block[:, j + 1:] += compensation

        # After processing this block, compensate columns in future blocks
        if col_end < cols:
            # Compute the block error: difference between original W and quantized in this block
            block_err = W[:, col_start:col_end] - Q[:, col_start:col_end] * scale.float().unsqueeze(1)
            hinv_diag = torch.diag(Hinv_block).clamp_min(1e-10)  # (block_cols,)
            # Cross-block Hessian inverse: (block_cols, remaining_cols)
            hinv_cross = Hinv[col_start:col_end, col_end:]
            # block_err / hinv_diag: (rows, block_cols) / (block_cols,) -> (rows, block_cols)
            # @ hinv_cross: (rows, block_cols) @ (block_cols, remaining_cols) -> (rows, remaining_cols)
...
def collect_hessians(model: nn.Module, val_tokens: Tensor, device: torch.device,
    ...
    Returns dict mapping module name -> Hessian tensor on CPU."""
    ...
    # Prepare calibration batches from validation tokens
    total_tokens = val_tokens.numel() - 1
    ...
    # Normalize by number of samples and move to CPU
    result = {}
...
def _state_dict_key_to_module_name(key: str) -> str:
    ...
    if key.endswith(".weight"):
        return key[:-7]  # remove '.weight'
    if key.endswith(".bias"):
        return key[:-5]  # remove '.bias'
    return key
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            # Try Full GPTQ if enabled and Hessian available
...
def main() -> None:
    ...
    log0("activation:leaky_relu_0.5_squared")

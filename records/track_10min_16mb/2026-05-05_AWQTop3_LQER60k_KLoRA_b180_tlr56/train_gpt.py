...
try:
    ...
    from flash_attn import flash_attn_func as flash_attn_3_func, flash_attn_varlen_func
    _HAS_FA3 = False
...
_COMPILER_DISABLE = (
    torch.compiler.disable if hasattr(torch, "compiler") else torch._dynamo.disable
)
_ROCM_CLANG = "/opt/rocm-6.2.4/llvm/bin/clang++"
_ROCM_CLANG_CC = "/opt/rocm-6.2.4/llvm/bin/clang"
_INDUCTOR_CONFIG = None

if _IS_ROCM:
    os.environ.setdefault("CC", _ROCM_CLANG_CC)
    os.environ.setdefault("CXX", _ROCM_CLANG)
    try:
        _INDUCTOR_CONFIG = __import__("torch._inductor.config", fromlist=["config"])
        _INDUCTOR_CONFIG.shape_padding = False
        cxx_candidates = []
        for candidate in (os.environ.get("CXX"), _ROCM_CLANG, "clang++", "g++"):
            if candidate and candidate not in cxx_candidates:
                cxx_candidates.append(candidate)
        _INDUCTOR_CONFIG.cpp.cxx = tuple(cxx_candidates)
    except Exception:
        _INDUCTOR_CONFIG = None
...
class Hyperparameters:
    ...
    if sparse_attn_gate_enabled and gate_attn_out:
        raise ValueError("SPARSE_ATTN_GATE_ENABLED=1 and GATE_ATTN_OUT=1 are mutually exclusive")
    # AWQ-lite (PR #1908): activation-aware mixed-precision GPTQ. Top-K most-salient 64-column
    # groups are promoted to INT8 inside the same Hessian-based GPTQ solve. Saliency = act_rms * |w|.
    ...
    # Skip tok_emb in AWQ promotion (forces matrix-only). Useful when tok_emb's
    # high act_rms saturates the top-K but its INT8 promotion bloats the artifact.
    awq_lite_skip_embed = bool(int(os.environ.get("AWQ_LITE_SKIP_EMBED", "0")))
    # Cap protected groups per tensor — forces AWQ to spread across many tensors
    # rather than dump everything into one. 0 = no per-tensor cap.
    awq_lite_per_tensor_max = int(os.environ.get("AWQ_LITE_PER_TENSOR_MAX", "0"))
    # Force stop training at exact step (PR #1908 step-match parity).
    force_stop_step = int(os.environ.get("FORCE_STOP_STEP", "0"))
    ...
    # LQER asymmetric rank-r correction on top-K GPTQ residual tensors (PR #1797).
    # Post-GPTQ SVD of residual E = W_fp − W_q; keep rank-r A=U[:,:r]·S[:r],
    # B=Vh[:r,:]; dequant reconstructs W_q + A @ B. Asym packing: A in INT2
    # per-matrix scalar, B in INT4 groupwise. ~30 KB / ~0.009 BPB recovery.
    ...
    # Adaptive LQER allocation: greedy byte-budget allocator over (tensor, rank)
    # pairs ranked by SVD energy / byte cost. Per-tensor rank is encoded in
    # meta string. Used only when LQER_ADAPTIVE=1; falls back to top_K x rank.
    lqer_adaptive = bool(int(os.environ.get("LQER_ADAPTIVE", "0")))
    lqer_budget_bytes = int(os.environ.get("LQER_BUDGET_BYTES", "60000"))
    # Accept any non-digit separator so sbatch --export comma parsing doesn't
    # mangle this. e.g. LQER_CAND_RANKS=4_6_8_10 works.
    lqer_cand_ranks = os.environ.get("LQER_CAND_RANKS", "4_6_8_10")
    ...
    datasets_dir = os.environ.get(
        "DATA_PATH", os.path.join(data_dir, "datasets", f"fineweb10B_sp{vocab_size}")
    )
    train_files = os.path.join(datasets_dir, "fineweb_train_[0-9]*.bin")
    val_files = os.path.join(datasets_dir, "fineweb_val_[0-9]*.bin")
...
def _piece_has_pue_marker(piece):
    return any(0xE000 <= ord(c) <= 0xF8FF for c in piece)
...
def build_sentencepiece_luts(sp, vocab_size, device):
    ...
    for token_id in range(sp_vocab_size):
        ...
        if _piece_has_pue_marker(piece):
    ...
    if pue_count > 0:
        print(f"build_sentencepiece_luts: {pue_count} PUE-marker tokens zeroed out")
    return (
...
def _flash_attn_varlen(q, k, v, cu_seqlens, max_seqlen):
...
if _IS_ROCM:
    _flash_attn_varlen = _COMPILER_DISABLE(_flash_attn_varlen)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.gate_attn_width = 24
        self.sparse_attn_gate = False
        self.sparse_attn_gate_scale = 1.0
        self.gate_window = 12
    ...
    def forward(self, x, q_w, k_w, v_w, out_w, cu_seqlens=None, max_seqlen=0):
        ...
        if cu_seqlens is not None:
            y = _flash_attn_varlen(q[0], k[0], v[0], cu_seqlens, max_seqlen)[None]
        else:
        ...
        if self.gate_attn_out:
            g = 2.0 * torch.sigmoid(self.attn_gate_proj(x[:, :, :self.gate_attn_width]))
            y = y * g.unsqueeze(-1)
        elif self.sparse_attn_gate:
            gate_in = x[..., : self.gate_window].contiguous()
            g = torch.sigmoid(self.sparse_attn_gate_scale * F.linear(gate_in, self.attn_gate_w.to(x.dtype)))
            y = y * g.unsqueeze(-1)
...
class MLP(nn.Module):
    def __init__(self, dim, mlp_mult):
        super().__init__()
        self.use_fused = _HAS_TENSOR_DESCRIPTOR
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        if h.gate_attn_out:
            for block in self.blocks:
                ...
                block.attn.gate_attn_width = h.gate_attn_width
                block.attn.attn_gate_proj = CastedLinear(
                    h.gate_attn_width, h.num_heads, bias=False
                )
                nn.init.zeros_(block.attn.attn_gate_proj.weight)
        if h.sparse_attn_gate_enabled:
            for block in self.blocks:
                block.attn.sparse_attn_gate = True
                block.attn.sparse_attn_gate_scale = h.sparse_attn_gate_scale
                block.attn.gate_window = h.gate_window
                W = torch.zeros(h.num_heads, h.gate_window, dtype=torch.float32)
                block.attn.attn_gate_w = nn.Parameter(W)
    ...
    def forward_logits(self, input_ids, cu_seqlens=None, max_seqlen=0):
        ...
        if self.smear_gate_enabled and x.dim() == 3 and x.size(1) > 1:
            ...
            # BOS mask (PR #1851/#1855): zero smear at positions where the
            # CURRENT token is BOS — prevents pulling info into a new doc's
            # opening token from the previous doc's last token.
            global BOS_ID
            if BOS_ID is None:
                BOS_ID = 1
            not_bos_cur = (input_ids[:, 1:] != BOS_ID).to(dtype=g.dtype).unsqueeze(-1)
            g = g * not_bos_cur
    ...
    def _block_with_lora(self, block, x, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w):
        ...
        if attn.gate_attn_out:
            g = 2.0 * torch.sigmoid(attn.attn_gate_proj(n[:, :, :attn.gate_attn_width]))
            y = y * g.unsqueeze(-1)
        elif attn.sparse_attn_gate:
            gate_in = n[..., : attn.gate_window].contiguous()
            g = torch.sigmoid(attn.sparse_attn_gate_scale * F.linear(gate_in, attn.attn_gate_w.to(n.dtype)))
            y = y * g.unsqueeze(-1)
        y = y.reshape(bsz, seqlen, dim)
...
# used in PR #1787's 1.06378 record). Replaces the stock (3.4445, -4.775,
# 2.0315) tuple applied 5x with 5 per-step optimal tuples. Same compute cost;
# tighter polar factor. Set POLAR_EXPRESS_NS=0 to restore stock Muon.
_PE_COEFFS = (
...
_STOCK_COEFFS = ((3.4445, -4.775, 2.0315),) * 5
_POLAR_EXPRESS_NS = os.environ.get("POLAR_EXPRESS_NS", "1") == "1"


@torch.compile(fullgraph=not _IS_ROCM)
def zeropower_via_newtonschulz5(G, steps=10, eps=1e-07):
    ...
    coeffs_src = _PE_COEFFS if _POLAR_EXPRESS_NS else _STOCK_COEFFS
    coeffs = coeffs_src[:steps] if steps <= len(coeffs_src) else coeffs_src
    for a, b, c in coeffs:
        A = X @ X.mT
        B = b * A + c * (A @ A)
        X = a * X + B @ X
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,parallel_post_lambdas,parallel_resid_lambdas,attn_gate_proj,attn_gate_w",
    ).split(",")
    if pattern
)
...
def gptq_quantize_weight(
    w, H, clip_sigmas=3.0, clip_range=63, block_size=128,
    protect_groups=None, group_size=None, protect_clip_range=None,
):
    ...
    sf_hi = None
    if (protect_groups and group_size is not None and protect_clip_range is not None
        ...
        if starts:
            ...
            s_hi = (clip_sigmas * row_std / protect_clip_range).clamp_min(1e-10).to(torch.float16)
            sf_hi = s_hi.float()
            protect_meta = {
    ...
    for i1 in range(0, cols, block_size):
        ...
        for j in range(i2 - i1):
            ...
            if protect_mask_perm is not None and bool(protect_mask_perm[i1 + j]):
                q_col = torch.clamp(torch.round(w_col / sf_hi), -protect_clip_range, protect_clip_range)
...
def _lqer_pack_asym(A, B, g=64):
    ...
    # B: INT4 groupwise over flattened B (signed [-8, 7], per-group scale).
    Bf = B.reshape(-1, g)
...
def _awq_lite_group_candidates(w, act_rms, group_size):
    ...
    for gi in range(n_groups):
        ...
        cands.append((float(saliency[start:end].sum()), start, end))
    return cands


def gptq_mixed_quantize(state_dict, hessians, h, act_stats=None):
    result = {}
    meta = {}
    lqer_on = bool(getattr(h, "lqer_enabled", False))
    awq_on = bool(getattr(h, "awq_lite_enabled", False)) and act_stats is not None
    lqer_cands = {}
    orig_by_name = {}
    awq_selected = collections.defaultdict(list)
    if awq_on:
        skip_embed = bool(getattr(h, "awq_lite_skip_embed", False))
        per_tensor_max = int(getattr(h, "awq_lite_per_tensor_max", 0))
        awq_cands = []
        for (name, tensor) in state_dict.items():
            t = tensor.detach().cpu().contiguous()
            if t.is_floating_point() and t.numel() > 65536 and name in act_stats:
                if skip_embed and "tok_emb" in name:
                    continue
        ...
        chosen = 0
        for (_score, name, start, end) in awq_cands:
            if chosen >= h.awq_lite_group_top_k:
                break
            if per_tensor_max > 0 and len(awq_selected[name]) >= per_tensor_max:
                continue
            awq_selected[name].append((start, end))
            chosen += 1
        if awq_selected:
            log(f"AWQ-lite: promoted {sum(len(v) for v in awq_selected.values())} group(s) "
                f"to int{h.awq_lite_bits} across {len(awq_selected)} tensor(s)")
    for (name, tensor) in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        ...
        q, s, protect_meta = gptq_quantize_weight(
            ...
            protect_clip_range=(2 ** (h.awq_lite_bits - 1) - 1) if name in awq_selected else None,
        )
        result[name + ".q"] = q
        result[name + ".scale"] = s
        meta[name] = f"gptq (int{bits})"
        # Compute reconstructed W_q (with AWQ-protected groups using high-precision scale).
        ...
        if protect_meta is not None:
            ...
            result[name + ".awqg_size"] = torch.tensor(protect_meta["size"], dtype=torch.int16)
            meta[name] = meta[name] + f"+awqgrpint{h.awq_lite_bits}"
            gsz = protect_meta["size"]
            for start in protect_meta["starts"].tolist():
                W_q[:, start : start + gsz] = (
                    q[:, start : start + gsz].float() * protect_meta["s_hi"].float().view(-1, 1)
                )
        if lqer_on:
            E = t.float() - W_q
    ...
    if lqer_on and lqer_cands:
        ...
        adaptive = bool(getattr(h, "lqer_adaptive", False))

        def _asym_bytes(rows, cols, r, group):
            # A: INT2, scalar fp16 → rows*r*2/8 + 2
            # B: INT4 groupwise → r*cols*4/8 + (r*cols/group)*2 (fp16 scales)
            a_bytes = rows * r * 2 // 8 + 2
            b_bytes = r * cols * 4 // 8
            n_groups = (r * cols) // group if group > 0 else 0
            b_bytes += n_groups * 2
            return a_bytes + b_bytes

        if adaptive:
            # Split on any non-digit so 4,6,8,10 / 4_6_8_10 / 4-6-8-10 all work.
            cand_ranks = sorted(
                {int(x) for x in re.findall(r"\d+", str(h.lqer_cand_ranks))}
            )
            if not cand_ranks:
                cand_ranks = [4, 6, 8, 10]
            budget = int(h.lqer_budget_bytes)
            # SVD all candidates once.
            svd_cache = {}
            options = []
            for (name, (E, frob)) in lqer_cands.items():
                ...
                for r in cand_ranks:
                    rr = min(r, S.numel())
                    energy = float((S[:rr] ** 2).sum().item())
                    cost = _asym_bytes(rows, cols, rr, asym_g)
                    # Marginal gain over previous rank choice for this tensor:
                    delta_energy = energy - prev_energy
                    if r > cand_ranks[0]:
                        prev_cost = _asym_bytes(rows, cols, min(cand_ranks[cand_ranks.index(r) - 1], S.numel()), asym_g)
                        delta_cost = max(cost - prev_cost, 1)
                    else:
                        delta_cost = max(cost, 1)
                    options.append(
                        (delta_energy / delta_cost, name, rr, energy, cost)
                    )
                    prev_energy = energy
            # Greedy selection: best gain-per-byte, only one rank per tensor (highest r selected).
            options.sort(key=lambda x: -x[0])
            best_for = {}  # name -> (rank, total_cost) for the highest-rank pick we can afford
            remaining = budget
            # Two-pass: first include base rank picks, then upgrade ranks if budget allows.
            for (gpb, name, r, energy, cost) in options:
                if name in best_for:
                    cur_r, cur_cost = best_for[name]
                    if r <= cur_r:
                        continue
                    extra = cost - cur_cost
                    if extra > remaining:
                        continue
                    best_for[name] = (r, cost)
                    remaining -= extra
                else:
                    if cost > remaining:
                        continue
                    ...
                    remaining -= cost
            log(
                f"LQER adaptive: budget={budget} bytes, used={budget - remaining} bytes, tensors={len(best_for)}"
            )
            for (name, (r, cost)) in sorted(best_for.items()):
                log(f"  {name}: rank={r} bytes={cost}")
            top = [
                (name, (lqer_cands[name][0], lqer_cands[name][1]))
                for name in best_for
            ]
            rank_for = {name: r for (name, (r, _)) in best_for.items()}
        else:
            top = sorted(lqer_cands.items(), key=lambda kv: -kv[1][1])[: h.lqer_top_k]
            rank_for = {name: h.lqer_rank for (name, _) in top}
            svd_cache = {}
        ...
        for (name, (E, _)) in top:
            ...
            if asym_on and B.numel() % asym_g == 0:
                ...
                meta[name] = meta[name] + f"+lqer_asym_r{r}"
            else:
                ...
                result[name + ".lqB"] = qB
                result[name + ".lqBs"] = sB
                meta[name] = meta[name] + f"+lqer_r{r}"
...
def dequantize_mixed(result, meta, template_sd):
    ...
    for (name, orig) in template_sd.items():
        ...
        if "lqer_asym" in info:
            ...
            qB = (
                qB_t.reshape(-1, g_sz).float() * sB_t.float().view(-1, 1)
            ).reshape(qB_t.shape)
            W = W + qA @ qB
        elif "lqer" in info:
...
# ── Per-group brotli compression (port of PR #1855 per-group, brotli-only) ──
import struct as _struct, collections as _coll
...
def _similarity_sort_l1(matrix):
    ...
    for _ in range(n - 1):
        ...
        dists = np.sum(np.abs(matrix[unused_idx].astype(np.float32) - cur), axis=1)
        best = unused_idx[np.argmin(dists)]
        order.append(best)
        used[best] = True
...
def _pack_streams(streams):
    n = len(streams)
    hdr = _PACK_MAGIC + _struct.pack("<I", n)
    for s in streams:
        hdr += _struct.pack("<I", len(s))
    return hdr + b"".join(streams)


def _unpack_streams(blob):
    assert blob[:4] == _PACK_MAGIC
    n = _struct.unpack("<I", blob[4:8])[0]
    off = 8
    lengths = [_struct.unpack("<I", blob[off + i*4:off + i*4 + 4])[0] for i in range(n)]
    off += n * 4
    streams = []
...
def _lrzip_compress_subproc(data):
    import subprocess as _sp, tempfile as _tf, os as _os
    with _tf.TemporaryDirectory() as td:
        inp = _os.path.join(td, "in.bin"); outp = inp + ".lrz"
        with open(inp, "wb") as f: f.write(data)
        _sp.run(["lrzip", "-z", "-L", "9", "-q", "-o", outp, inp], check=True, capture_output=True)
        with open(outp, "rb") as f: return f.read()


def _lrzip_decompress_subproc(data):
    import subprocess as _sp, tempfile as _tf, os as _os
    with _tf.TemporaryDirectory() as td:
        inp = _os.path.join(td, "in.lrz"); outp = _os.path.join(td, "out.bin")
        with open(inp, "wb") as f: f.write(data)
        _sp.run(["lrzip", "-d", "-f", "-q", "-o", outp, inp], check=True, capture_output=True)
        with open(outp, "rb") as f: return f.read()


def _serialize_pergroup_lrzip(quant_result, quant_meta, num_layers):
    import brotli as _brotli
    groups = _coll.defaultdict(list)
    remainder = {}
    for name, t in sorted(quant_result.items()):
        if t.dtype != torch.int8:
            remainder[name] = t; continue
        parts = name.split(".")
        routed = False
        if parts[0] == "blocks" and parts[1].isdigit():
            key = ".".join(parts[2:])
            if key in _GROUP_ORDER:
                groups[key].append((int(parts[1]), t)); routed = True
        else:
            group_key = "_" + name
            if group_key in _GROUP_ORDER:
                groups[group_key] = [(0, t)]; routed = True
        if not routed:
            remainder[name] = t
    ...
    for group_key in _GROUP_ORDER:
        ...
        streams.append(_lrzip_compress_subproc(blob))

    remainder_buf = io.BytesIO()
    torch.save({"r": remainder, "m": quant_meta, "s": shape_manifest}, remainder_buf)
    streams.append(_brotli.compress(remainder_buf.getvalue(), quality=11, lgwin=24))
    streams.append(_brotli.compress(all_perms, quality=11) if all_perms else b"")
    return _pack_streams(streams)


def _deserialize_pergroup_lrzip(blob, num_layers):
    import brotli as _brotli
    streams = _unpack_streams(blob)
    n_groups = len(_GROUP_ORDER)
    remainder_state = torch.load(io.BytesIO(_brotli.decompress(streams[n_groups])), map_location="cpu")
    quant_meta = remainder_state["m"]
    quant_result = dict(remainder_state["r"])
    shape_manifest = remainder_state["s"]
    all_perms = _brotli.decompress(streams[n_groups + 1]) if streams[n_groups + 1] else b""

    raw_groups = {}
    for i, gk in enumerate(_GROUP_ORDER):
        data = streams[i]
        raw_groups[gk] = _lrzip_decompress_subproc(data) if data else b""
...
def _serialize_pergroup_brotli(quant_result, quant_meta, num_layers):
    import brotli as _brotli
    groups = _coll.defaultdict(list)
    remainder = {}
    for name, t in sorted(quant_result.items()):
    ...
    for group_key in _GROUP_ORDER:
        ...
        for idx, t in tensors:
            ...
            grp_shapes.append(orig_shape)
        shape_manifest[group_key] = grp_shapes
        streams.append(_brotli.compress(blob, quality=11, lgwin=24))
...
def _deserialize_pergroup_brotli(blob, num_layers):
    import brotli as _brotli
    streams = _unpack_streams(blob)
    n_groups = len(_GROUP_ORDER)

    remainder_state = torch.load(
        io.BytesIO(_brotli.decompress(streams[n_groups])), map_location="cpu"
    )
    quant_meta = remainder_state["m"]
    quant_result = dict(remainder_state["r"])
    shape_manifest = remainder_state["s"]
    ...
    for i, gk in enumerate(_GROUP_ORDER):
        data = streams[i]
        raw_groups[gk] = _brotli.decompress(data) if data else b""
...
def serialize(h, base_model, code):
    ...
    log(f"GPTQ:collected {len(hessians)} Hessians + {len(act_stats)} act-stats in {time.perf_counter()-t0:.1f}s")
    quant_result, quant_meta = gptq_mixed_quantize(sd_cpu, hessians, h, act_stats=act_stats)
    if h.compressor == "pergroup":
        log("Serialize: per-group brotli compression...")
        t1 = time.perf_counter()
        quant_blob = _serialize_pergroup_brotli(quant_result, quant_meta, h.num_layers)
        log(f"Serialize: per-group compression done in {time.perf_counter()-t1:.1f}s")
    elif h.compressor == "pergroup_lrzip":
        log("Serialize: per-group lrzip compression...")
        t1 = time.perf_counter()
        quant_blob = _serialize_pergroup_lrzip(quant_result, quant_meta, h.num_layers)
        log(f"Serialize: per-group lrzip done in {time.perf_counter()-t1:.1f}s")
    else:
...
def deserialize(h, device):
    ...
    if quant_blob_disk[:4] == _PACK_MAGIC:
        _streams_check = _unpack_streams(quant_blob_disk)
        _is_lrzip = False
        for _s in _streams_check:
            if _s and _s[:4] == b"LRZI":
                _is_lrzip = True
                break
        if _is_lrzip:
            log("Deserialize: per-group lrzip decompression...")
            t0 = time.perf_counter()
            qr, qm = _deserialize_pergroup_lrzip(quant_blob_disk, h.num_layers)
        else:
            log("Deserialize: per-group brotli decompression...")
            t0 = time.perf_counter()
            qr, qm = _deserialize_pergroup_brotli(quant_blob_disk, h.num_layers)
        log(f"Deserialize: per-group decompression done in {time.perf_counter()-t0:.1f}s")
        quant_state = {"w": qr, "m": qm}
    else:
...
def train_model(h, device, val_data):
    ...
    if _DISABLE_COMPILE:
        ...
        compiled_forward_logits = torch.compile(
            base_model.forward_logits, dynamic=False, fullgraph=False
        )
    ...
    forced_stop_step = int(getattr(h, "force_stop_step", 0))
    torch.cuda.synchronize()
    t0 = time.perf_counter()
    step = 0
    while True:
        last_step = (
            ...
            or (forced_stop_step > 0 and step >= forced_stop_step)
        )
        should_validate = (
...
def train_and_eval(h, device):
    ...
    _TTT_EVAL_ONLY = os.environ.get("TTT_EVAL_ONLY", "0") == "1"
    if _TTT_EVAL_ONLY:
        artifact_override = os.environ.get("TTT_EVAL_ONLY_ARTIFACT", "").strip()
        if artifact_override:
            h.quantized_model_path = artifact_override
        log(
            f"TTT_EVAL_ONLY=1 — skipping train/GPTQ/quant eval; loading "
            f"artifact={h.quantized_model_path}"
        )
        base_model = None
        compiled_model = None
    ...
    _DISABLE_COMPILE = (
        os.environ.get("DISABLE_COMPILE", "0") == "1"
        or os.environ.get("DISABLE_EVAL_COMPILE", "0") == "1"
    )
    if _DISABLE_COMPILE:
        compiled_model = eval_model
        compiled_forward_logits = eval_model.forward_logits
    else:
        compiled_model = torch.compile(eval_model, dynamic=False, fullgraph=False)
        compiled_forward_logits = torch.compile(
            eval_model.forward_logits, dynamic=False, fullgraph=False
        )
    if not _TTT_EVAL_ONLY:
        timed_eval(
    ...
    if h.ttt_enabled:
        ...
        _DISABLE_TTT_COMPILE = (
            os.environ.get("DISABLE_COMPILE", "0") == "1"
            or os.environ.get("DISABLE_TTT_COMPILE", "0") == "1"
        )
        # dynamic=True caused stride/shape assertions on ROCm inductor; force
        # dynamic=False so compile specializes per concrete (bsz, seq_len).
        _TTT_COMPILE_DYNAMIC = (
            os.environ.get("TTT_COMPILE_DYNAMIC", "0") == "1"
        )
        _TTT_FIXED_SEQ_COMPILE = (
            os.environ.get("TTT_FIXED_SEQ_COMPILE", "0") == "1"
        )
        _TTT_COMPILE_BUCKET_SIZE = int(
            os.environ.get("TTT_COMPILE_BUCKET_SIZE", "128" if _IS_ROCM else "0")
        )
        _fwd_ttt_compiled_inners = {}

        def _get_fwd_ttt_compiled(bsz, seq_len):
            key = ("dynamic",) if _TTT_COMPILE_DYNAMIC else (int(bsz), int(seq_len))
            compiled = _fwd_ttt_compiled_inners.get(key)
            if compiled is None:
                compiled = torch.compile(_fwd_ttt_inner, dynamic=_TTT_COMPILE_DYNAMIC)
                _fwd_ttt_compiled_inners[key] = compiled
            return compiled

        def _fwd_ttt(input_ids, target_ids, lora):
            if _DISABLE_TTT_COMPILE:
                return _fwd_ttt_inner(input_ids, target_ids, lora=lora)
            orig_len = input_ids.size(1)
            target_len = orig_len
            if _TTT_FIXED_SEQ_COMPILE:
                target_len = h.ttt_eval_seq_len
            elif _TTT_COMPILE_BUCKET_SIZE > 0:
                target_len = min(
                    h.ttt_eval_seq_len,
                    ((orig_len + _TTT_COMPILE_BUCKET_SIZE - 1) // _TTT_COMPILE_BUCKET_SIZE)
                    * _TTT_COMPILE_BUCKET_SIZE,
                )
            if target_len > orig_len:
                pad_len = target_len - orig_len
                input_ids = F.pad(input_ids, (0, pad_len), value=0)
                target_ids = F.pad(target_ids, (0, pad_len), value=0)
            compiled = _get_fwd_ttt_compiled(input_ids.size(0), input_ids.size(1))
            out = compiled(input_ids, target_ids, lora=lora)
            return out[:, :orig_len] if target_len > orig_len else out
        ...
        for bsz in warmup_bszes:
            ...
            warmup_ctx_lens = (
                (h.ttt_eval_seq_len,)
                if _TTT_FIXED_SEQ_COMPILE and not _DISABLE_TTT_COMPILE
                else (
                    (
                        min(
                            h.ttt_eval_seq_len,
                            ((h.ttt_chunk_size + _TTT_COMPILE_BUCKET_SIZE - 1)
                            // _TTT_COMPILE_BUCKET_SIZE)
                            * _TTT_COMPILE_BUCKET_SIZE,
                        ),
                        h.ttt_eval_seq_len,
                    )
                    if _TTT_COMPILE_BUCKET_SIZE > 0 and not _DISABLE_TTT_COMPILE
                    else (h.ttt_chunk_size, h.ttt_eval_seq_len)
                )
            )
            for ctx_len in sorted(set(warmup_ctx_lens)):
...
def main():
    ...
    if distributed:
        dist.init_process_group(
            ...
            timeout=datetime.timedelta(
                hours=float(os.environ.get("DIST_TIMEOUT_HOURS", "12"))
            ),
        )
        dist.barrier()
    ...
    torch._dynamo.config.cache_size_limit = int(
        os.environ.get("DYNAMO_CACHE_SIZE_LIMIT", "256" if _IS_ROCM else "64")
    )
    # Allow disabling inductor shape/stride assertions so the compiled kernel
    # runs even when its guessed shape pattern (e.g. ROCm-specific stride
    # expressions) doesn't match the actual runtime shape. Set
    # TORCHINDUCTOR_SIZE_ASSERTS=0 to silence.
    if os.environ.get("TORCHINDUCTOR_SIZE_ASSERTS", "1") == "0":
